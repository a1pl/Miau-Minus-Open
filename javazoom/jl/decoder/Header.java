...
public final class Header {
        ...
        try {
            ...
            if (xing.equals(new String(tmp))) {
                ...
                if ((flags[3] & 1) != 0) {
                    ...
                    this.h_vbr_frames = tmp[0] << 24 & 0xFF000000
                        | tmp[1] << 16 & 0xFF0000
                        | tmp[2] << 8 & 0xFF00
                        | tmp[3] & 255;
                    length += 4;
                ...
                if ((flags[3] & 2) != 0) {
                    System.arraycopy(firstframe, var9 + length, tmp, 0, tmp.length);
                    this.h_vbr_bytes = tmp[0] << 24 & 0xFF000000
                        | tmp[1] << 16 & 0xFF0000
                        | tmp[2] << 8 & 0xFF00
                        | tmp[3] & 255;
                    length += 4;
                ...
                if ((flags[3] & 8) != 0) {
                    System.arraycopy(firstframe, var9 + length, tmp, 0, tmp.length);
                    this.h_vbr_scale = tmp[0] << 24 & 0xFF000000
                        | tmp[1] << 16 & 0xFF0000
                        | tmp[2] << 8 & 0xFF00
                        | tmp[3] & 255;
                    length += 4;
    ...
    public int calculate_framesize() {
        if (this.h_layer == 1) {
            this.framesize = 12
                * bitrates[this.h_version][0][this.h_bitrate_index]
            ...
            if (this.h_layer == 3) {
                if (this.h_version == 1) {
                    this.nSlots = this.framesize
                        - (this.h_mode == 3 ? 17 : 32)
                        - (this.h_protection_bit != 0 ? 0 : 2)
                        - 4;
                } else {
                    this.nSlots = this.framesize
                        - (this.h_mode == 3 ? 9 : 17)
    ...
    public int max_number_of_frames(int streamsize) {
        if (this.h_vbr) {
            ...
            return this.framesize + 4 - this.h_padding_bit == 0
                ? 0
                : streamsize / (this.framesize + 4 - this.h_padding_bit);
        }
    }
    ...
    public int min_number_of_frames(int streamsize) {
        if (this.h_vbr) {
            ...
            return this.framesize + 5 - this.h_padding_bit == 0
                ? 0
                : streamsize / (this.framesize + 5 - this.h_padding_bit);
        }
    }

    public float ms_per_frame() {
        if (!this.h_vbr) {
            float[][] ms_per_frame_array = new float[][]{
    ...
    public String bitrate_string() {
        return this.h_vbr
            ? Integer.toString(this.bitrate() / 1000) + " kb/s"
            : bitrate_str[this.h_version][this.h_layer - 1][this.h_bitrate_index];
    }
