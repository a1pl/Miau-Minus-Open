        ...
        if (Filename != null && (BitsPerSample == 8 || BitsPerSample == 16) && NumChannels >= 1 && NumChannels <= 2) {
            ...
            if (retcode == 0) {
                ...
                if (retcode == 0) {
                    ...
                    retcode = this.Write((int)this.wave_format.data.nSamplesPerSec, 4);
                    retcode = this.Write((int)this.wave_format.data.nAvgBytesPerSec, 4);
    ...
    class WaveFormat_Chunk {
        ...
        public int VerifyValidity() {
            boolean ret = this.header.ckID == RiffFile.FourCC("fmt ")
                ...
                && this.data.nAvgBytesPerSec
                    == this.data.nChannels * this.data.nSamplesPerSec * this.data.nBitsPerSample / 8
