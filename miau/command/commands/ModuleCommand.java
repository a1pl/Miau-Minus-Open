...
public class ModuleCommand extends Command {
    public ModuleCommand() {
        super(
            new ArrayList<>(
                Miau.moduleManager.modules.values().stream().map(Module::getName).collect(Collectors.toList())
            )
        );
    }
    ...
    public void runCommand(ArrayList<String> args) {
        ...
        if (args.size() >= 2) {
            ...
            if (property == null) {
                ...
                ChatUtil.display(
                    "%sInvalid value for property &o%s&r (%s)&r", property.getName(), property.getValuePrompt()
                );
            }
        } else {
            List<Property<?>> properties = Miau.propertyManager.properties.get(module.getClass());
            if (properties != null) {
                List<Property<?>> visible = properties.stream()
