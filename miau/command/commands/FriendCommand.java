...
public class FriendCommand extends Command {
    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() >= 2) {
            ...
            switch (subCommand) {
                ...
                case "add":
                    if (args.size() < 3) {
                        ChatUtil.display(
                            "%sUsage: .%s add <&oname&r> [&oname&r] ...&r", args.get(0).toLowerCase(Locale.ROOT)
                        );
                        return;
                    }
                ...
                case "remove":
                    if (args.size() < 3) {
                        ChatUtil.display(
                            String.format(
                                "%sUsage: .%s remove <&oname&r> [&oname&r] ...&r", args.get(0).toLowerCase(Locale.ROOT)
                            )
                        );
                        return;
                    }
                ...
                default:
                    if (args.size() == 2) {
                        if (Miau.friendManager.isFriend(args.get(1))) {
                            this.runCommand(new ArrayList<>(Arrays.asList(args.get(0), "remove", args.get(1))));
                        } else {
                            this.runCommand(new ArrayList<>(Arrays.asList(args.get(0), "add", args.get(1))));
                        }
        ...
        ChatUtil.display(
            "%sUsage: .%s <&oa(dd)&r/&or(emove)&r/&ol(ist)&r/&oc(lear)&r>&r", args.get(0).toLowerCase(Locale.ROOT)
        );
    }
}
