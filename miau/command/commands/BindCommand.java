...
public class BindCommand extends Command {
    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() < 3) {
            if (args.size() == 2 && (args.get(1).equalsIgnoreCase("l") || args.get(1).equalsIgnoreCase("list"))) {
                List<Module> modules = Miau.moduleManager
                    .modules
                    .values()
                    .stream()
                    .filter(modulex -> modulex.getKey() != 0)
            ...
            if (!keyInput.equalsIgnoreCase("NONE")
                ...
                if (keyIndex == 0) {
                    int buttonIndex = this.getMouseButtonIndex(keyInput);
                    if (buttonIndex != -1) {
    ...
    private int getMouseButtonIndex(String buttonName) {
        if (buttonName.startsWith("MOUSE")) {
            ...
            } catch (NumberFormatException | StringIndexOutOfBoundsException var5) {
            }
        }
