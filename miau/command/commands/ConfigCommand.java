    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() < 2) {
            ...
            switch (sub) {
                ...
                case "list":
                    try {
                        File[] configs = new File("./config/Miau/").listFiles(FILE_FILTER);
                        if (configs == null) {
                            throw new Exception();
                        }

                        if (configs.length == 0) {
                            ChatUtil.display("%sNo configs found (&o%s&r)&r", "./config/Miau/");
                        }
                        ...
                        for (File file : configs) {
                            ...
                            String config = String.format(
                                ".config load %s", FilenameUtils.removeExtension(file.getName())
                            );
                            ChatUtil.send(
                                new ChatComponentText(formatted)
                                    .func_150255_a(
                                        new ChatStyle()
                                            .func_150241_a(new ClickEvent(Action.RUN_COMMAND, config))
                                            .func_150209_a(
                                                new HoverEvent(
                        ...
                        ChatUtil.display("%sFailed to read (&o%s&r)&r", "./config/Miau/");
                    }
                ...
                case "directory":
                    try {
                        Desktop.getDesktop().open(new File("./config/Miau/"));
                    } catch (Exception e) {
                        ChatUtil.display("%sFailed to open (&o%s&r)&r", "./config/Miau/");
                    }
