...
public class Chams extends Module {
    ...
    private boolean shouldRenderChams(EntityLivingBase entityLivingBase) {
        ...
        if (mc.func_175606_aa().func_70032_d(entityLivingBase) > 512.0F) {
            return false;
        }

        if (entityLivingBase instanceof EntityPlayer) {
            if (entityLivingBase != mc.field_71439_g && entityLivingBase != mc.func_175606_aa()) {
                if (TeamUtil.isBot((EntityPlayer)entityLivingBase)) {
                    return this.bots.getValue();
                } else {
                    return TeamUtil.isFriend((EntityPlayer)entityLivingBase)
            ...
            if (!(entityLivingBase instanceof EntityMob) && !(entityLivingBase instanceof EntitySlime)) {
                return (
                        entityLivingBase instanceof EntityAnimal
                ...
                return entityLivingBase instanceof EntityEnderman
                    ? this.enderman.getValue()
                    : entityLivingBase instanceof EntityBlaze ? this.blaze.getValue() : this.mobs.getValue();
            }
        } else {
            return !entityLivingBase.func_82150_aj() && this.bosses.getValue();
        }
    }
    ...
    public void onTick(TickEvent event) {
        if (this.isEnabled() && mc.field_71441_e != null) {
            for (EntityPlayer player : mc.field_71441_e.field_73010_i) {
                if (this.shouldRenderChams(player)) {
