...
public class AutoAuth extends Module {
    ...
    private static final String[] PASSWORD_TOKENS = new String[]{"password", "pass"};
    ...
    public void onPacket(PacketEvent event) {
        if (this.isEnabled() && event.getType() == EventType.RECEIVE && event.getPacket() instanceof S02PacketChat) {
            ...
            AutoAuth.AuthPrompt prompt = AutoAuth.AuthPrompt.from(message);
            if (prompt != null) {
                int repeat = Math.max(1, this.countPasswordSlots(message));
                this.queue(prompt.buildCommand(this.password.getValue(), repeat));
            }
        }
    }
    ...
    public void onTick(TickEvent event) {
        if (this.isEnabled() && event.getType() == EventType.PRE && this.queuedCommand != null) {
            if (System.currentTimeMillis() - this.queuedAt >= this.delay.getValue().intValue()) {
                if (mc.field_71439_g != null) {
                    mc.field_71439_g.func_71165_d(this.queuedCommand);
                }
    ...
    private enum AuthPrompt {
        ...
        private static AutoAuth.AuthPrompt from(String message) {
            String normalized = message.toLowerCase(Locale.ROOT);

            for (AutoAuth.AuthPrompt prompt : values()) {
