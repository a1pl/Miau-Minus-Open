...
public class MiauAPI {
    ...
    private static final String API_V1 = "https://api.getmiau.today/api/v1";
    ...
    public static String rpcConfig() throws Exception {
        return get("https://api.getmiau.today/api/rpc", "Miau/RPC");
    }

    public static String getClientVersion() throws Exception {
        return get("https://api.getmiau.today/api/v1/client/version", "Miau/Version");
    }

    public static String getClientNotification() throws Exception {
        return get("https://api.getmiau.today/api/v1/client/notification", "Miau/Notification");
    }
    ...
    public static boolean isOutdated(String current, String latest) {
        if (current != null && latest != null) {
            ...
            for (int i = 0; i < length; i++) {
                int curr = i < currParts.length && !currParts[i].isEmpty() ? Integer.parseInt(currParts[i]) : 0;
                int lat = i < latestParts.length && !latestParts[i].isEmpty() ? Integer.parseInt(latestParts[i]) : 0;
                if (curr < lat) {
                    return true;
                }
    ...
    private static String clientPath(String path) throws Exception {
        return "https://api.getmiau.today/api/v1/client/" + encode("myau") + "/" + path;
    }
    ...
    private static String formatError(int code, String body) {
        if (body != null && !body.isEmpty()) {
            String text = body.replaceAll("(?is)<style.*?</style>", " ")
