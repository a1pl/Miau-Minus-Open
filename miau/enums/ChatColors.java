...
public enum ChatColors {
    ...
    public static final String PREFIX_CLEAN = "Miau Minus » ";
    ...
    public static ChatColors getClosestColor(Color target) {
        ChatColors closest = WHITE;
        ...
        for (ChatColors color : values()) {
            if (color != MAGIC
                ...
                && color != ITALIC
                && color != RESET) {
                Color c = new Color(color.toAwtColor());
                double dist = Math.pow(c.getRed() - target.getRed(), 2.0)
                    + Math.pow(c.getGreen() - target.getGreen(), 2.0)
                    + Math.pow(c.getBlue() - target.getBlue(), 2.0);
                if (dist < minDistance) {
                    minDistance = dist;
                    closest = color;
                }
    ...
    public static String formatColor(String string) {
        ...
        for (int i = 0; i < cArray.length - 1; i++) {
            if (cArray[i] == '&' && "0123456789AaBbCcDdEeFfKkLlMmNnOoRr".indexOf(cArray[i + 1]) > -1) {
                cArray[i] = 167;
