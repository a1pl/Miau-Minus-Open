...
public class FloatProperty extends Property<Float> {
    ...
    public FloatProperty(
        String string, Float value, Float secondValue, Float minimum, Float maximum, BooleanSupplier check
    ) {
        super(string, value, floatV -> floatV >= minimum && floatV <= maximum, check);
        this.minimum = minimum;
        this.maximum = maximum;
    ...
    public String formatValue() {
        return this.doubleSlider
            ? String.format("&6%s-%s", this.getValue(), this.secondValue)
            : String.format("&6%s", this.getValue());
    }

    @Override
    public boolean parseString(String string) {
        if (this.doubleSlider && string.contains("-")) {
    ...
    public boolean read(JsonObject jsonObject) {
        ...
        if (this.doubleSlider && jsonObject.has(this.getName() + "_second")) {
    ...
    public boolean setValue(Object object) {
        ...
        if (this.doubleSlider && this.secondValue != null && val > this.secondValue) {
            val = this.secondValue;
        }
