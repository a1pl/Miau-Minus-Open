            ...
            public Appendable append(CharSequence csq, int start, int end) throws IOException {
                ...
                if (index < end) {
                    ...
                    if (this.pendingHighSurrogate != -1) {
                        ...
                        char[] escaped = UnicodeEscaper.this.escape(
                            Character.toCodePoint((char)this.pendingHighSurrogate, c)
                        );
                        if (escaped != null) {
                ...
                if (this.pendingHighSurrogate != -1) {
                    if (!Character.isLowSurrogate(c)) {
                        throw new IllegalArgumentException(
                            "Expected low surrogate character but got '" + c + "' with value " + c
                        );
                    }
                    ...
                    if (Character.isLowSurrogate(c)) {
                        throw new IllegalArgumentException(
                            "Unexpected low surrogate character '" + c + "' with value " + c
                        );
                    }
    ...
    protected static final int codePointAt(CharSequence seq, int index, int end) {
        if (index < end) {
            ...
            if (c1 < '\ud800' || c1 > '\udfff') {
                ...
                throw new IllegalArgumentException(
                    "Unexpected low surrogate character '" + c1 + "' with value " + c1 + " at index " + (index - 1)
                );
            } else if (index == end) {
                return -c1;
            } else {
                ...
                if (Character.isLowSurrogate(c2)) {
                    ...
                    throw new IllegalArgumentException(
                        "Expected low surrogate but got char '" + c2 + "' with value " + c2 + " at index " + index
                    );
                }
            }
        } else {
