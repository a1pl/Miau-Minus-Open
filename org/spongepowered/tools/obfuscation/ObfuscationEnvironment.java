...
public abstract class ObfuscationEnvironment implements IObfuscationEnvironment {
        ...
        if (!this.initDone) {
            ...
            if (this.inFileNames == null) {
                this.ap
                    .printMessage(
                        Kind.ERROR,
                        "The "
                            + this.type.getConfig().getInputFileOption()
            ...
            for (String inputFileName : this.inFileNames) {
                ...
                try {
                    if (inputFile.isFile()) {
                        this.ap
                            .printMessage(
                                Kind.NOTE, "Loading " + this.type + " mappings from " + inputFile.getAbsolutePath()
                            );
            ...
            if (successCount < 1) {
                this.ap
                    .printMessage(
                        Kind.ERROR,
                        "No valid input files for " + this.type + " could be read, processing may not be sucessful."
                    );
        ...
        if (obfd == null && method.isFullyQualified()) {
            ...
            if (type != null && !type.isImaginary()) {
                ...
                String superClassName = ((TypeElement)((DeclaredType)superClass).asElement())
                    .getQualifiedName()
                    .toString();
                return this.getObfMethod(
                    new MemberInfo(method.name, superClassName.replace('.', '/'), method.desc, method.matchAll)
                );
            } else {
                return null;
            }
    ...
    public MappingMethod getObfMethod(MappingMethod method, boolean lazyRemap) {
        ...
        if (remappedOwner == null
            || remappedOwner.equals(method.getOwner())
            || remappedOwner.equals(mapping.getOwner())) {
    ...
    public MappingField getObfField(MappingField field, boolean lazyRemap) {
        ...
        String remappedOwner = this.getObfClass(mapping.getOwner());
        if (remappedOwner != null
            && !remappedOwner.equals(field.getOwner())
            && !remappedOwner.equals(mapping.getOwner())) {
            return mapping.move(remappedOwner);
        } else {
    ...
    final class RemapperProxy implements ObfuscationUtil.IClassRemapper {
        ...
        public String map(String typeName) {
            return ObfuscationEnvironment.this.mappingProvider == null
                ? null
                : ObfuscationEnvironment.this.mappingProvider.getClassMapping(typeName);
        }
