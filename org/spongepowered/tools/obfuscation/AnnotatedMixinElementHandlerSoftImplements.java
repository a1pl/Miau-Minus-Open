...
public class AnnotatedMixinElementHandlerSoftImplements extends AnnotatedMixinElementHandler {
    ...
    private void addInterfaceMethodMapping(
        Interface.Remap remap, TypeHandle iface, String prefix, MethodHandle method, String name, String desc
    ) {
        ...
        if (obfData.isEmpty()) {
            if (remap.forceRemap()) {
                this.ap
                    .printMessage(
                        Kind.ERROR, "No obfuscation mapping for soft-implementing method", method.getElement()
                    );
            }
        } else {
