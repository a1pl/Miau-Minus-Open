...
abstract class AnnotatedMixinElementHandler {
    ...
    protected final void addFieldMapping(
        ObfuscationType type,
        AnnotatedMixinElementHandler.ShadowElementName name,
        String mcpSignature,
        String obfSignature
    ) {
    ...
    protected final void addFieldMapping(
        ObfuscationType type, String mcpName, String obfName, String mcpSignature, String obfSignature
    ) {
    ...
    protected final void addMethodMapping(
        ObfuscationType type,
        AnnotatedMixinElementHandler.ShadowElementName name,
        String mcpSignature,
        String obfSignature
    ...
    protected final void validateTarget(
        Element element, AnnotationHandle annotation, AnnotatedMixinElementHandler.AliasedElementName name, String type
    ) {
        ...
        for (TypeHandle target : this.mixin.getTargets()) {
            if (!target.isImaginary()) {
                ...
                if (targetMethod != null) {
                    ...
                    this.printMessage(
                        Kind.WARNING, "Cannot find target for " + type + " method in " + target, method, annotation
                    );
                }
            }
        }
    ...
    private void validateMethodVisibility(
        ExecutableElement method,
        AnnotationHandle annotation,
        String type,
        TypeHandle target,
        ...
        if (visTarget != null) {
            ...
            if (visTarget.ordinal() > visMethod.ordinal()) {
                this.printMessage(
                    Kind.WARNING, visMethod + " " + type + " method cannot reduce " + visibility, method, annotation
                );
            } else if (visTarget == Visibility.PRIVATE && visMethod.ordinal() > visTarget.ordinal()) {
                this.printMessage(
                    Kind.WARNING, visMethod + " " + type + " method will upgrade " + visibility, method, annotation
                );
            }
        }
    }
    ...
    protected final void validateReferencedTarget(
        ExecutableElement method, AnnotationHandle inject, MemberInfo reference, String type
    ) {
            ...
            if (!target.isImaginary()) {
                ...
                if (targetMethod == null) {
                    this.ap
                        .printMessage(
                            Kind.WARNING,
                            "Cannot find target method for " + type + " in " + target,
                            method,
                            inject.asMirror()
                        );
                }
