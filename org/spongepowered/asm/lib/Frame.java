...
class Frame {
        ...
        switch (desc.charAt(index)) {
            ...
            default:
                ...
                switch (desc.charAt(dims)) {
                    ...
                    case 'Y':
                    default:
                        String tx = desc.substring(dims + 1, desc.length() - 1);
                        data = 24117248 | cw.addType(tx);
                        break;
                    case 'F':
                        data = 16777218;
                        break;
    ...
    private int pop() {
        return this.outputStackTop > 0
            ? this.outputStack[--this.outputStackTop]
            : 50331648 | -(--this.owner.inputStackTop);
    }
    ...
    private void pop(int elements) {
        if (this.outputStackTop >= elements) {
            ...
            this.owner.inputStackTop = this.owner.inputStackTop - (elements - this.outputStackTop);
            this.outputStackTop = 0;
        }
    }
    ...
    void execute(int opcode, int arg, ClassWriter cw, Item item) {
        switch (opcode) {
            ...
            case 58: {
                ...
                this.set(arg, t1);
                if (arg > 0) {
                    int t2x = this.get(arg - 1);
                    if (t2x != 16777220 && t2x != 16777219) {
                        if ((t2x & 251658240) != 16777216) {
                            this.set(arg - 1, t2x | 8388608);
                        }
                    } else {
            ...
            case 189:
                String sx = item.strVal1;
                this.pop();
                if (sx.charAt(0) == '[') {
                    this.push(cw, '[' + sx);
                } else {
                    this.push(292552704 | cw.addType(sx));
                }
                break;
            case 192:
    ...
    final boolean merge(ClassWriter cw, Frame frame, int edge) {
        ...
        if (edge > 0) {
            for (int var15 = 0; var15 < nLocal; var15++) {
                int t = this.inputLocals[var15];
                changed |= merge(cw, t, frame.inputLocals, var15);
            }
            ...
            for (int var13 = 0; var13 < nInputStack; var13++) {
                int t = this.inputStack[var13];
                if (this.initializations != null) {
                    t = this.init(cw, t);
                }

                changed |= merge(cw, t, frame.inputStack, var13);
            }

            for (int var14 = 0; var14 < this.outputStackTop; var14++) {
                int s = this.outputStack[var14];
                int dim = s & -268435456;
                int kind = s & 251658240;
                int t;
                if (kind == 16777216) {
                ...
                changed |= merge(cw, t, frame.inputStack, nInputStack + var14);
            }
