...
class MethodWriter extends MethodVisitor {
    ...
    MethodWriter(
        ClassWriter cw, int access, String name, String desc, String signature, String[] exceptions, int compute
    ) {
        super(327680);
        ...
        if (this.compute != 0) {
            if (this.compute == 1) {
                if (this.currentBlock.frame == null) {
                    ...
                    this.currentBlock
                        .frame
                        .initInputFrame(this.cw, this.access, Type.getArgumentTypes(this.descriptor), nLocal);
                ...
                for (int i = 0; i < nLocal; i++) {
                    if (local[i] instanceof String) {
                        ...
                        this.frame[frameIndex++] = 25165824
    ...
    public void visitJumpInsn(int opcode, Label label) {
        ...
        if (this.currentBlock != null) {
            if (this.compute == 0) {
                ...
                this.stackSize = this.stackSize + Frame.SIZE[opcode];
            ...
            if (this.compute == 0) {
                if (this.currentBlock != null) {
                    if (label.position == this.currentBlock.position) {
                        this.currentBlock.status = this.currentBlock.status | label.status & 16;
                ...
                if (this.previousBlock != null) {
                    if (label.position == this.previousBlock.position) {
                        this.previousBlock.status = this.previousBlock.status | label.status & 16;
    ...
    final void put(ByteVector out) {
        ...
        if (this.classReaderOffset != 0) {
            ...
            if (this.code.length > 0) {
                ...
                if (this.cattrs != null) {
                    size += this.cattrs
                        .getSize(this.cw, this.code.data, this.code.length, this.maxStack, this.maxLocals);
                }
