...
public class ClassReader {
        ...
        for (int i = 1; i < ll; i++) {
            ...
            switch (tag) {
                ...
                case 11: {
                    ...
                    item.set(
                        tag, this.readClass(index, buf), this.readUTF8(nameType, buf), this.readUTF8(nameType + 2, buf)
                    );
                    break;
                }
                case 12:
                ...
                case 18: {
                    ...
                    int nameType = this.items[this.readUnsignedShort(index + 2)];
                    item.set(
                        this.readUTF8(nameType, buf), this.readUTF8(nameType + 2, buf), this.readUnsignedShort(index)
                    );
                }
            }
    ...
    private void copyBootstrapMethods(ClassWriter classWriter, Item[] items, char[] c) {
        ...
        if (found) {
            ...
            while (j < boostrapMethodCount) {
                ...
                item.set(position, hashCode & 2147483647);
    ...
    private void readCode(MethodVisitor mv, Context context, int u) {
        ...
        for (int i = this.readUnsignedShort(u); i > 0; i--) {
            ...
            if ("LocalVariableTable".equals(attrName)) {
                if ((context.flags & 2) == 0) {
                    ...
                    while (j > 0) {
                        ...
                        if (labels[label] == null) {
                            this.readLabel(label, labels).status |= 1;
                        }
                ...
                if ("StackMapTable".equals(attrName)) {
                    ...
                    for (int j = 0; j < context.attrs.length; j++) {
                        if (context.attrs[j].type.equals(attrName)) {
                            Attribute attr = context.attrs[j]
                                .read(this, u + 8, this.readInt(u + 4), c, codeStart - 8, labels);
                            if (attr != null) {
                                attr.next = attributes;
                                attributes = attr;
                            }
                ...
                nitoff = itanns.length != 0 && this.readByte(itanns[0]) >= 67
        ...
        while (u < codeEnd) {
            ...
            while (tanns != null && tann < tanns.length && ntoff <= offset) {
                ...
                tann++;
                ntoff = tann < tanns.length && this.readByte(tanns[tann]) >= 67
            ...
            while (itanns != null && itann < itanns.length && nitoff <= offset) {
                ...
                itann++;
                nitoff = itann < itanns.length && this.readByte(itanns[itann]) >= 67
        ...
        for (int i = 0; i < offsets.length; i++) {
            ...
            if (target >>> 24 == 66) {
                ...
                u = this.readAnnotationValues(
                    u + 2, c, true, mv.visitTryCatchAnnotation(target, path, this.readUTF8(u, c), visible)
                );
            } else {
    ...
    private int readAnnotationValue(int v, char[] buf, String name, AnnotationVisitor av) {
        if (av == null) {
            byte var26;
            switch (this.b[var26] & 0xFF) {
                case 64:
                    return this.readAnnotationValues(var26 + 3, buf, true, null);
                case 91:
                    return this.readAnnotationValues(var26 + 1, buf, false, null);
                case 101:
                    return var26 + 5;
                default:
                    return var26 + 3;
            ...
            switch (this.b[v++] & 0xFF) {
                ...
                case 90:
                    av.visit(
                        name, this.readInt(this.items[this.readUnsignedShort(v)]) == 0 ? Boolean.FALSE : Boolean.TRUE
                    );
                    v += 2;
                    break;
                case 91:
    ...
    private int readFrame(int stackMap, boolean zip, boolean unzip, Context frame) {
        ...
        if (tag < 64) {
            ...
            if (tag == 247) {
                ...
                frame.localCount = frame.localCount - frame.localDiff;
                frame.stackCount = 0;
            } else if (tag == 251) {
                frame.mode = 3;
                frame.stackCount = 0;
                ...
                while (n > 0) {
                    stackMap = this.readFrameType(frame.stack, local++, stackMap, c, labels);
                    n--;
                }
            }
        }
    ...
    private Attribute readAttribute(
        Attribute[] attrs, String type, int off, int len, char[] buf, int codeOff, Label[] labels
    ) {
    ...
    public short readShort(int index) {
        byte[] b = this.b;
        return (short)((b[index] & 255) << 8 | b[index + 1] & 0xFF);
    }
    ...
    public long readLong(int index) {
        ...
        long l0 = this.readInt(index + 4) & 4294967295L;
        return l1 << 32 | l0;
    }
