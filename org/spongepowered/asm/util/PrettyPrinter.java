        ...
        if (!(object instanceof Iterable)) {
            if (object instanceof Map) {
                ...
                return object.getClass().isArray()
                    ? this.add((Object[])object, indent + "%s")
                    : this.add("%s%s", margin, object);
            }
        } else {
        ...
        if (this.recalcWidth) {
            ...
            for (Object line : this.lines) {
                if (line instanceof PrettyPrinter.IVariableWidthEntry) {
                    this.width = Math.min(
                        4096, Math.max(this.width, ((PrettyPrinter.IVariableWidthEntry)line).getWidth())
                    );
                }
            }
        }
        ...
        public String toString() {
            ...
            for (int col = 0; col < args.length; col++) {
                ...
                if (col >= this.args.length) {
                    ...
                    args[col] = this.args[col].length() > column.getMaxWidth()
                        ? this.args[col].substring(0, column.getMaxWidth())
                        : this.args[col];
                }
            }
