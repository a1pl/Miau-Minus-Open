...
public class SignaturePrinter {
    ...
    public SignaturePrinter(String name, Type returnType, Type[] argTypes, String[] argNames) {
        ...
        if (this.argTypes.length > this.argNames.length) {
            throw new IllegalArgumentException(
                String.format(
                    "Types array length must not exceed names array length! (names=%d, types=%d)",
                    this.argNames.length,
                    this.argTypes.length
                )
            );
        }
    }
    ...
    public String toString() {
        return this.appendArgs(new StringBuilder().append(this.modifiers).append(" ").append(this.name), false, true)
            .toString();
    }
            ...
            if (this.argTypes[var] != null) {
                ...
                try {
                    String name = typesOnly
                        ? null
                        : (Strings.isNullOrEmpty(this.argNames[var]) ? "unnamed" + var : this.argNames[var]);
