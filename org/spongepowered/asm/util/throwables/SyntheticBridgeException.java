...
public class SyntheticBridgeException extends MixinException {
    ...
    public SyntheticBridgeException(
        SyntheticBridgeException.Problem problem,
        String name,
        String desc,
        int index,
        AbstractInsnNode a,
    ...
    public void printAnalysis(IMixinContext context, MethodNode mda, MethodNode mdb) {
        ...
        this.printMethod(
                printer.add("Incoming method").add().kv("Owner", context.getClassRef().replace('/', '.')).add(), mdb
            )
            .hr();
    ...
    private PrettyPrinter printProblem(PrettyPrinter printer, IMixinContext context, MethodNode mda, MethodNode mdb) {
        ...
        switch (this.problem) {
            ...
            case BAD_CAST:
                printer.add(
                    "Incompatible CHECKCAST encountered at opcode "
                        + this.index
                        + ", this could indicate that the bridge"
                );
            ...
            case BAD_INVOKE_DESC:
                ...
                if (arga.length != argb.length) {
                    ...
                    String winner = arga.length == argCount
                        ? "The TARGET"
                        : (argb.length == argCount ? " The INCOMING" : "NEITHER");
                    ...
                    printer.add(
                        "the target method has %d arguments, the incoming method has %d. This is an",
                        arga.length,
                        argb.length
                    );
    ...
    public enum Problem {
        ...
        BAD_INVOKE_NAME(
            "Conflicting synthetic bridge target method name in synthetic bridge method %1$s%2$s Existing:%6$s Incoming:%7$s"
        ),
        BAD_INVOKE_DESC(
            "Conflicting synthetic bridge target method descriptor in synthetic bridge method %1$s%2$s Existing:%8$s Incoming:%9$s"
        ),
        BAD_LENGTH(
            "Mismatched bridge method length for synthetic bridge method %1$s%2$s unexpected extra opcode at offset %3$d"
        );
