...
public class CallbackInjector extends Injector {
    ...
    protected void sanityCheck(Target target, List<InjectionPoint> injectionPoints) {
        ...
        if (target.isStatic != this.isStatic) {
            throw new InvalidInjectionException(
                this.info, "'static' modifier of callback method does not match target in " + this
            );
        }
        ...
        if ("<init>".equals(target.method.name)) {
            for (InjectionPoint injectionPoint : injectionPoints) {
                if (!injectionPoint.getClass().equals(BeforeReturn.class)) {
                    throw new InvalidInjectionException(
                        this.info,
                        "Found injection point type "
                            + injectionPoint.getClass().getSimpleName()
                            + " targetting a ctor in "
                            + this
                            + ". Only RETURN allowed for a ctor target"
                    );
                }
            }
        }
    ...
    protected void inject(Target target, InjectionNodes.InjectionNode node) {
        ...
        this.inject(
            new CallbackInjector.Callback(this.methodNode, target, node, locals, this.localCapture.isCaptureLocals())
        );
    }
        ...
        if (this.localCapture.isPrintLocals()) {
            ...
            if (callback.checkDescriptor(this.methodNode.desc)) {
                ...
                if (callback.canCaptureLocals) {
                    MethodNode surrogateHandler = Bytecode.findMethod(
                        this.classNode, this.methodNode.name, callback.getDescriptor()
                    );
                    label39:
                    if (surrogateHandler != null && Annotations.getVisible(surrogateHandler, Surrogate.class) != null) {
                        ...
                        switch (this.localCapture) {
                            case CAPTURE_FAILEXCEPTION:
                                ...
                                callbackMethod = this.generateErrorMethod(
                                    callback,
                    ...
                    label63: {
                        ...
                        if (callback.checkDescriptor(returnableSig)) {
                            throw new InvalidInjectionException(
                                this.info,
                                "Invalid descriptor on " + this.info + "! CallbackInfoReturnable is required!"
                            );
                        }
                        ...
                        if (surrogateHandler != null
                            && Annotations.getVisible(surrogateHandler, Surrogate.class) != null) {
                            callbackMethod = surrogateHandler;
                            break label63;
                        }
    ...
    private MethodNode generateErrorMethod(CallbackInjector.Callback callback, String errorClass, String message) {
        MethodNode method = this.info
            .addMethod(this.methodNode.access, this.methodNode.name + "$missing", callback.getDescriptor());
        method.maxLocals = Bytecode.getFirstNonArgLocalIndex(
            Type.getArgumentTypes(callback.getDescriptor()), !this.isStatic
        );
        method.maxStack = 3;
    ...
    private void printLocals(CallbackInjector.Callback callback) {
        ...
        SignaturePrinter handlerSig = new SignaturePrinter(
        ...
        printer.kv(
            "Instruction",
            "%s %s",
            callback.node.getClass().getSimpleName(),
    ...
    protected void instanceCallbackInfo(CallbackInjector.Callback callback, String id, String desc, boolean store) {
        ...
        if (store) {
            callback.target
                .addLocalVariable(
                    this.callbackInfoVar, "callbackInfo" + this.callbackInfoVar, "L" + this.callbackInfoClass + ";"
                );
        ...
        if (this.cancellable) {
            ...
            callback.add(
                new MethodInsnNode(
                    ...
                    CallbackInfo.getIsCancelledMethodName(),
                    CallbackInfo.getIsCancelledMethodSig(),
                    false
                )
            );
    ...
    private class Callback extends InsnList {
        ...
        Callback(
            ...
            this.isAtReturn = this.node.getCurrentTarget() instanceof InsnNode
                && this.isValueReturnOpcode(this.node.getCurrentTarget().getOpcode());
            this.desc = target.getCallbackDescriptor(this.localTypes, target.arguments);
            this.descl = target.getCallbackDescriptor(
                true, this.localTypes, target.arguments, this.frameSize, this.extraArgs
            );
            this.invoke = target.arguments.length
                + (this.canCaptureLocals ? this.localTypes.length - this.frameSize : 0);
        }
        ...
        String getDescriptorWithAllLocals() {
            return this.target
                .getCallbackDescriptor(true, this.localTypes, this.target.arguments, this.frameSize, 32767);
        }

        String getCallbackInfoConstructorDescriptor() {
            return this.isAtReturn
                ? CallbackInfo.getConstructorDescriptor(this.target.returnType)
                : CallbackInfo.getConstructorDescriptor();
        }
