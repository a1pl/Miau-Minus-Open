...
public class RedirectInjector extends InvokeInjector {
            ...
            if (node.getCurrentTarget() instanceof MethodInsnNode) {
                ...
                if (!(node.getCurrentTarget() instanceof TypeInsnNode) || node.getCurrentTarget().getOpcode() != 187) {
                    throw new InvalidInjectionException(
                        this.info,
                        String.format(
                            "%s annotation on is targetting an invalid insn in %s in %s",
                            this.annotationType,
                            target,
                            this
                        )
                ...
                if (!this.isStatic && target.isStatic) {
                    throw new InvalidInjectionException(
                        this.info,
                        String.format("non-static callback method %s has a static target which is not supported", this)
                    );
                }
    ...
    protected void postInject(Target target, InjectionNodes.InjectionNode node) {
        ...
        if (node.getOriginalTarget() instanceof TypeInsnNode && node.getOriginalTarget().getOpcode() == 187) {
            ...
            if (meta.wildcard && meta.injected == 0) {
                throw new InvalidInjectionException(
                    this.info, String.format("%s ctor invocation was not found in %s", this.annotationType, target)
                );
            }
        }
    }
    ...
    protected void injectAtInvoke(Target target, InjectionNodes.InjectionNode node) {
        RedirectInjector.RedirectedInvoke invoke = new RedirectInjector.RedirectedInvoke(
            target, (MethodInsnNode)node.getCurrentTarget()
        );
    ...
    protected void validateParams(RedirectInjector.RedirectedInvoke invoke) {
        ...
        if (!invoke.returnType.equals(this.returnType)) {
            throw new InvalidInjectionException(
                this.info,
                String.format(
                    "%s has an invalid signature. Expected return type %s found %s",
                    description,
                    this.returnType,
                    invoke.returnType
                )
        ...
        for (int index = 0; index < argc; index++) {
            ...
            if (fromType.equals(toType)) {
                if (coerce != null && this.info.getContext().getOption(MixinEnvironment.Option.DEBUG_VERBOSE)) {
                    Injector.logger
                        .warn(
                            "Redundant @Coerce on {} argument {}, {} is identical to {}",
                            new Object[]{description, index, toType, fromType}
                        );
                }
            } else {
    ...
    private void injectAtFieldAccess(Target target, InjectionNodes.InjectionNode node) {
        ...
        if (handlerDimensions > targetDimensions) {
            throw new InvalidInjectionException(
                this.info, "Dimensionality of handler method is greater than target array on " + this
            );
        }
    ...
    private void injectAtArrayField(
        Target target, FieldInsnNode fieldNode, int opCode, Type ownerType, Type fieldType, int fuzz, int opcode
    ) {
        Type elementType = fieldType.getElementType();
        if (opCode != 178 && opCode != 180) {
            throw new InvalidInjectionException(
                this.info,
                String.format("Unspported opcode %s for array access %s", Bytecode.getOpcodeName(opCode), this.info)
            );
        }
        ...
        if (this.returnType.getSort() != 0) {
            ...
            AbstractInsnNode varNode = BeforeFieldAccess.findArrayNode(
                target.insns, fieldNode, elementType.getOpcode(79), fuzz
            );
            this.injectAtSetArray(target, fieldNode, varNode, ownerType, fieldType);
        }
    }

    private void injectAtGetArray(
        Target target, FieldInsnNode fieldNode, AbstractInsnNode varNode, Type ownerType, Type fieldType
    ) {
    ...
    public void injectArrayRedirect(
        Target target, FieldInsnNode fieldNode, AbstractInsnNode varNode, boolean withArgs, String type
    ) {
        if (varNode == null) {
            String advice = "";
            throw new InvalidInjectionException(
    ...
    public void injectAtScalarField(Target target, FieldInsnNode fieldNode, int opCode, Type ownerType, Type fieldType) {
        ...
        if (opCode != 178 && opCode != 180) {
            if (opCode != 179 && opCode != 181) {
                throw new InvalidInjectionException(
                    this.info, String.format("Unspported opcode %s for %s", Bytecode.getOpcodeName(opCode), this.info)
                );
            }
    ...
    private AbstractInsnNode injectAtGetField(
        InsnList insns, Target target, FieldInsnNode node, boolean staticField, Type owner, Type fieldType
    ) {
        String handlerDesc = staticField
            ? Bytecode.generateDescriptor(fieldType)
            : Bytecode.generateDescriptor(fieldType, owner);
    ...
    private AbstractInsnNode injectAtPutField(
        InsnList insns, Target target, FieldInsnNode node, boolean staticField, Type owner, Type fieldType
    ) {
        String handlerDesc = staticField
            ? Bytecode.generateDescriptor(null, fieldType)
            : Bytecode.generateDescriptor(null, owner, fieldType);
    ...
    protected boolean checkDescriptor(String desc, Target target, String type) {
        if (this.methodNode.desc.equals(desc)) {
            ...
            String alternateDesc = String.format(
                "%s%s%s", desc.substring(0, pos), Joiner.on("").join(target.arguments), desc.substring(pos)
            );
            if (this.methodNode.desc.equals(alternateDesc)) {
                ...
                throw new InvalidInjectionException(
                    this.info,
                    String.format(
                        "%s method %s %s has an invalid signature. Expected %s but found %s",
                        this.annotationType,
                        type,
                        this,
                        desc,
    ...
    protected void injectAtConstructor(Target target, InjectionNodes.InjectionNode node) {
        ...
        if (meta == null) {
            throw new InvalidInjectionException(
                this.info,
                String.format(
                    "%s ctor redirector has no metadata, the injector failed a preprocessing phase",
                    this.annotationType
                )
            );
        }
        ...
        if (initNode == null) {
            ...
            if (isAssigned) {
                ...
                this.throwException(
                    ...
                    String.format(
                        "%s constructor handler %s returned null for %s",
                        this.annotationType,
                        this,
    ...
    private static String getGetArrayHandlerDescriptor(AbstractInsnNode varNode, Type returnType, Type fieldType) {
        return varNode != null && varNode.getOpcode() == 190
            ? Bytecode.generateDescriptor(Type.INT_TYPE, getArrayArgs(fieldType, 0))
            : Bytecode.generateDescriptor(returnType, getArrayArgs(fieldType, 1));
    }
    ...
    static class RedirectedInvoke {
        ...
        RedirectedInvoke(Target target, MethodInsnNode node) {
            ...
            this.locals = node.getOpcode() == 184
                ? this.args
                : (Type[])ObjectArrays.concat(Type.getType("L" + node.owner + ";"), this.args);
        }
    }
}
