...
public class ModifyConstantInjector extends RedirectInjector {
        ...
        if (this.preInject(node)) {
            ...
            if (targetNode instanceof JumpInsnNode) {
                ...
                throw new InvalidInjectionException(
                    this.info,
                    this.annotationType + " annotation is targetting an invalid insn in " + target + " in " + this
                );
            }
        }
    }
    ...
    private void injectExpandedConstantModifier(Target target, JumpInsnNode jumpNode) {
        ...
        if (opcode >= 155 && opcode <= 158) {
            ...
            throw new InvalidInjectionException(
                this.info,
                this.annotationType
                    + " annotation selected an invalid opcode "
        ...
        if (pop != null) {
            if (pop instanceof FieldInsnNode) {
                ...
                this.checkNarrowing(
                    ...
                    String.format(
                        "%s %s %s.%s",
                        Bytecode.getOpcodeName(pop),
                        SignaturePrinter.getTypeName(fieldType, false),
                        fieldNode.owner.replace('/', '.'),
                        fieldNode.name
                    )
                );
                ...
                this.checkNarrowing(
                    ...
                    target.returnType,
                    target.indexOf(pop),
                    "RETURN " + SignaturePrinter.getTypeName(target.returnType, false)
                );
    ...
    private void checkNarrowing(
        Target target, AbstractInsnNode constNode, Type constantType, Type type, int index, String description
    ) {
        ...
        if (toSort < fromSort) {
            ...
            String message = toSort == 1
                ? ". Implicit conversion to <boolean> can cause nondeterministic (JVM-specific) behaviour!"
                : "";
