...
public class BeforeFieldAccess extends BeforeInvoke {
    ...
    public BeforeFieldAccess(InjectionPointData data) {
        ...
        this.arrOpcode = "get".equalsIgnoreCase(array)
            ? 46
            : ("set".equalsIgnoreCase(array) ? 79 : ("length".equalsIgnoreCase(array) ? 190 : 0));
    ...
    protected boolean matchesInsn(AbstractInsnNode insn) {
        if (insn instanceof FieldInsnNode && (((FieldInsnNode)insn).getOpcode() == this.opcode || this.opcode == -1)) {
            if (this.arrOpcode == 0) {
                ...
                return insn.getOpcode() != 178 && insn.getOpcode() != 180
                    ? false
                    : Type.getType(((FieldInsnNode)insn).desc).getSort() == 9;
            }
        } else {
            return false;
        }
    ...
    protected boolean addInsn(InsnList insns, Collection<AbstractInsnNode> nodes, AbstractInsnNode insn) {
        if (this.arrOpcode > 0) {
            ...
            this.log(
                "{} > > > > searching for array access opcode {} fuzz={}",
                this.className,
                Bytecode.getOpcodeName(accOpcode),
                this.fuzzFactor
            );
    ...
    public static AbstractInsnNode findArrayNode(InsnList insns, FieldInsnNode fieldNode, int opcode, int searchRange) {
        ...
        for (AbstractInsnNode insn : insns) {
            ...
            if (insn instanceof FieldInsnNode) {
                ...
                if (field.desc.equals(fieldNode.desc)
                    && field.name.equals(fieldNode.name)
                    && field.owner.equals(fieldNode.owner)) {
                    return null;
                }
            }
