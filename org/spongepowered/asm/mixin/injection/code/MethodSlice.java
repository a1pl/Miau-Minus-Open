...
public final class MethodSlice {
    ...
    private MethodSlice(ISliceContext owner, String id, InjectionPoint from, InjectionPoint to) {
        if (from == null && to == null) {
            throw new InvalidSliceException(
                owner, String.format("%s is redundant. No 'from' or 'to' value specified", this)
            );
        }
    ...
    public ReadOnlyInsnList getSlice(MethodNode method) {
        ...
        if (start > end) {
            throw new InvalidSliceException(
                this.owner, String.format("%s is negative size. Range(%d -> %d)", this.describe(), start, end)
            );
        } else if (start < 0 || end < 0 || start > max || end > max) {
            throw new InjectionError(
                "Unexpected critical error in "
                    + this
                    + ": out of bounds start="
                    + start
                    + " end="
    ...
    private int find(
        MethodNode method, InjectionPoint injectionPoint, int defaultValue, int failValue, String description
    ) {
        ...
        if (nodes.size() != 1 && select == InjectionPoint.Selector.ONE) {
            throw new InvalidSliceException(
                this.owner,
                String.format("%s requires 1 result but found %d", this.describe(description), nodes.size())
            );
        }
        ...
        if (!result) {
            ...
            return method.instructions
                .indexOf(select == InjectionPoint.Selector.FIRST ? nodes.getFirst() : nodes.getLast());
        }
    }
    ...
    static final class InsnListSlice extends ReadOnlyInsnList {
        ...
        public ListIterator<AbstractInsnNode> iterator(int index) {
            return new MethodSlice.InsnListSlice.SliceIterator(
                super.iterator(this.start + index), this.start, this.end, this.start + index
            );
        }
