...
class MixinApplicatorInterface extends MixinApplicatorStandard {
    ...
    protected void applyFields(MixinTargetContext mixin) {
        for (Entry<FieldNode, ClassInfo.Field> entry : mixin.getShadowFields()) {
            ...
            this.logger
                .error("Ignoring redundant @Shadow field {}:{} in {}", new Object[]{shadow.name, shadow.desc, mixin});
        }
            ...
            try {
                ...
                if (injectInfo != null) {
                    throw new InvalidInterfaceMixinException(
                        mixin, injectInfo + " is not supported on interface mixin method " + method.name
                    );
                }
