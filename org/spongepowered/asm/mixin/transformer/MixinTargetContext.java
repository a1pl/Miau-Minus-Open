...
public class MixinTargetContext extends ClassContext implements IMixinContext {
    ...
    MixinTargetContext(MixinInfo mixin, ClassNode classNode, TargetClassContext context) {
        ...
        this.inheritsFromMixin = mixin.getClassInfo().hasMixinInHierarchy()
        ...
        if (Annotations.getInvisible(method, SoftOverride.class) != null) {
            ClassInfo.Method superMethod = this.targetClassInfo
                .findMethodInHierarchy(
                    method.name, method.desc, ClassInfo.SearchType.SUPER_CLASSES_ONLY, ClassInfo.Traversal.SUPER
                );
    ...
    private void transformFieldRef(MethodNode method, Iterator<AbstractInsnNode> iter, MemberRef fieldRef) {
        ...
        if (fieldRef.getOwner().equals(this.getClassRef())) {
            ...
            if (field != null
                && field.isRenamed()
                && field.getOriginalName().equals(fieldRef.getName())
                && field.isStatic()) {
                        ...
                        if (shadowField.isDecoratedFinal()) {
                            if (shadowField.isDecoratedMutable()) {
                                if (this.mixin
                                    .getParent()
                                    .getEnvironment()
                                    .getOption(MixinEnvironment.Option.DEBUG_VERBOSE)) {
                                    logger.warn(
                                        "Write access to @Mutable @Final field {} in {}::{}",
                                        new Object[]{shadowField, this.mixin, method.name}
                                    );
                                }
                            } else if (!"<init>".equals(method.name) && !"<clinit>".equals(method.name)) {
                                logger.error(
                                    "Write access detected to @Final field {} in {}::{}",
                                    new Object[]{shadowField, this.mixin, method.name}
                                );
                                if (this.mixin
                                    .getParent()
                                    .getEnvironment()
                                    .getOption(MixinEnvironment.Option.DEBUG_VERIFY)) {
                                    throw new InvalidMixinException(
                                        this.mixin,
                                        "Write access detected to @Final field "
                                            + shadowField
                                            + " in "
                                            + this.mixin
                                            + "::"
                                ...
                                logger.warn(
                                    "@Final field {} in {} should be final", new Object[]{shadowField, this.mixin}
                                );
                            }
                        }
    ...
    private void transformTypeNode(
        MethodNode method, Iterator<AbstractInsnNode> iter, TypeInsnNode typeInsn, AbstractInsnNode lastNode
    ) {
    ...
    private void transformInvokeDynamicNode(
        MethodNode method, Iterator<AbstractInsnNode> iter, InvokeDynamicInsnNode dynInsn
    ) {
    ...
    private void processImaginarySuper(MethodNode method, FieldInsnNode fieldInsn) {
        if (fieldInsn.getOpcode() != 180) {
            if ("<init>".equals(method.name)) {
                throw new InvalidMixinException(
                    this,
                    "Illegal imaginary super declaration: field " + fieldInsn.name + " must not specify an initialiser"
                );
            } else {
                throw new InvalidMixinException(
                    this,
                    "Illegal imaginary super access: found "
                        + Bytecode.getOpcodeName(fieldInsn.getOpcode())
                        + " opcode in "
                        + method.name
                        + method.desc
                );
            ...
            if (Annotations.getInvisible(method, SoftOverride.class) == null) {
                throw new InvalidMixinException(
                    this,
                    "Illegal imaginary super access: method "
                        + method.name
                        + method.desc
            ...
            for (AbstractInsnNode insn : method.instructions) {
                if (insn instanceof MethodInsnNode) {
                    ...
                    if (methodNode.owner.equals(this.getClassRef())
                        && methodNode.name.equals(method.name)
            ...
            throw new InvalidMixinException(
                this, "Illegal imaginary super access: could not find INVOKE for " + method.name + method.desc
            );
        } else {
            throw new InvalidMixinException(
                this, "Illegal imaginary super access: method " + method.name + method.desc + " is private or static"
            );
        }
    }
    ...
    private String transformDescriptor(Type type) {
        return type.getSort() == 11
            ? this.transformMethodDescriptor(type.getDescriptor())
            : this.transformSingleDescriptor(type);
    }
    ...
    private String transformSingleDescriptor(String desc, boolean isObject) {
        ...
        return !typeInfo.isMixin() ? desc : desc.replace(type, this.findRealType(typeInfo).toString());
    }
    ...
    public void postApply(String transformedName, ClassNode targetClass) {
        try {
            ...
            throw new InjectionError(
                String.format(
                    "Critical injection failure: Callback group %s in %s failed injection check: %s",
                    group,
                    this.mixin,
                    ex.getMessage()
                )
