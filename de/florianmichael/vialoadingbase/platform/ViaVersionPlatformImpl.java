...
public class ViaVersionPlatformImpl implements ViaPlatform<UserConnection> {
    ...
    public VLBTask runRepeatingAsync(Runnable runnable, long ticks) {
        return new VLBTask(
            Via.getManager().getScheduler().scheduleRepeating(runnable, 0L, ticks * 50L, TimeUnit.MILLISECONDS)
        );
    }
    ...
    public JsonObject getDump() {
        return ViaLoadingBase.getInstance().getDumpSupplier() == null
            ? new JsonObject()
            : ViaLoadingBase.getInstance().getDumpSupplier().get();
    }
}
