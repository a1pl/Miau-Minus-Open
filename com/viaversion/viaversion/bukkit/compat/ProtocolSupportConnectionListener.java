...
final class ProtocolSupportConnectionListener extends PacketListener {
    ...
    public void onPacketReceiving(PacketEvent event) {
        try {
            if (HANDSHAKE_PACKET_CLASS.isInstance(event.getPacket())
                && GET_VERSION_METHOD.invoke(this.connection) == PROTOCOL_VERSION_MINECRAFT_FUTURE) {
                Object packet = event.getPacket();
                int protocolVersion = (Integer)HANDSHAKE_PACKET_CLASS.getDeclaredMethod(
                        ProtocolSupportCompat.handshakeVersionMethod().methodName()
                    )
                    .invoke(packet);
    ...
    static {
        try {
            ...
            REMOVE_PACKET_LISTENER_METHOD = connectionClass.getDeclaredMethod(
                "removePacketListener", packetListenerClass
            );
        } catch (ReflectiveOperationException e) {
            throw new RuntimeException(e);
        }
