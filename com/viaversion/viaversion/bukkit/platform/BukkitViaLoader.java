...
public class BukkitViaLoader implements ViaPlatformLoader {
    ...
    public void load() {
        ...
        if (!Via.getAPI().getServerVersion().isKnown()) {
            Via.getPlatform()
                .getLogger()
                .severe("Server version has not been loaded yet, cannot register additional listeners");
        } else {
            ...
            if (serverProtocolVersion.olderThan(ProtocolVersion.v1_14)) {
                boolean use1_9Fix = plugin.getConf().is1_9HitboxFix()
                    && serverProtocolVersion.olderThan(ProtocolVersion.v1_9);
                if (use1_9Fix || plugin.getConf().is1_14HitboxFix()) {
                    try {
                        ...
                        Via.getPlatform()
                            .getLogger()
                            .log(Level.WARNING, "Could not load hitbox fix - please report this on our GitHub", e);
                    }
                }
            }
            ...
            if (serverProtocolVersion.olderThan(ProtocolVersion.v1_12)
                && !Boolean.getBoolean("com.viaversion.ignorePaperBlockPlacePatch")) {
                boolean paper = true;
            ...
            if (serverProtocolVersion.olderThan(ProtocolVersion.v1_19_4)
                && plugin.getConf().isArmorToggleFix()
                && this.hasGetHandMethod()) {
            ...
            if (serverProtocolVersion.olderThan(ProtocolVersion.v1_9)) {
                Via.getManager()
                    .getProviders()
                    .use(MovementTransmitterProvider.class, new BukkitViaMovementTransmitter());
                Via.getManager()
                    .getProviders()
                    .use(
                        HandItemProvider.class,
                        new HandItemProvider() {
                            @Override
                            public Item getHandItem(UserConnection info) {
                                if (BukkitViaLoader.this.handItemCache != null) {
                                    return BukkitViaLoader.this.handItemCache
                                        .getHandItem(info.getProtocolInfo().getUuid());
                                }

                                try {
                                    return (Item)Bukkit.getScheduler()
                                        .callSyncMethod(Bukkit.getPluginManager().getPlugin("ViaVersion"), () -> {
                                            UUID playerUUID = info.getProtocolInfo().getUuid();
                                            Player player = Bukkit.getPlayer(playerUUID);
                                            return player != null
                                                ? HandItemCache.convert(player.getItemInHand())
                                                : null;
                                        })
            ...
            if (serverProtocolVersion.olderThan(ProtocolVersion.v1_12) && plugin.getConf().is1_12QuickMoveActionFix()) {
                Via.getManager()
                    .getProviders()
                    .use(InventoryQuickMoveProvider.class, new BukkitInventoryQuickMoveProvider());
            }
