...
public class ChunkSectionType1_9 extends Type<ChunkSection> {
    ...
    public ChunkSection read(ByteBuf buffer) throws Exception {
        ...
        ChunkSection chunkSection = bitsPerBlock != 13
            ? new ChunkSectionImpl(true, paletteLength)
            : new ChunkSectionImpl(true);
        ...
        if (blockData.length > 0) {
            int expectedLength = (int)Math.ceil(4096 * bitsPerBlock / 64.0);
            if (blockData.length == expectedLength) {
                CompactArrayUtil.iterateCompactArray(
                    bitsPerBlock,
                    4096,
                    blockData,
                    bitsPerBlock == 13 ? blockPalette::setIdAt : blockPalette::setPaletteIndexAt
                );
            }
        }
    ...
    public void write(ByteBuf buffer, ChunkSection chunkSection) throws Exception {
        ...
        long[] data = CompactArrayUtil.createCompactArray(
            bitsPerBlock, 4096, bitsPerBlock == 13 ? blockPalette::idAt : blockPalette::paletteIndexAt
        );
