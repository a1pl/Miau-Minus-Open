...
public class StringType extends Type<String> {
    ...
    public String read(ByteBuf buffer) throws Exception {
        ...
        Preconditions.checkArgument(
            string.length() <= this.maxLength,
            "Cannot receive string longer than Short.MAX_VALUE characters (got %s bytes)",
    ...
    public void write(ByteBuf buffer, String object) throws Exception {
        if (object.length() > this.maxLength) {
            throw new IllegalArgumentException(
                "Cannot send string longer than Short.MAX_VALUE characters (got " + object.length() + " characters)"
            );
        }
