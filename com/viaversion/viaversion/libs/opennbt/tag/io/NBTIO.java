...
public final class NBTIO {
    ...
    public static <T extends Tag> T readTag(
        DataInput in, TagLimiter tagLimiter, boolean named, @Nullable Class<T> expectedTagType
    ) throws IOException {
        int id = in.readByte();
        if (expectedTagType != null && expectedTagType != TagRegistry.getClassFor(id)) {
            throw new IOException(
                "Expected tag type "
                    + expectedTagType.getSimpleName()
                    + " but got "
                    + TagRegistry.getClassFor(id).getSimpleName()
            );
        }
