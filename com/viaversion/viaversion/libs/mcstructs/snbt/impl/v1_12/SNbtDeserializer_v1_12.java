...
public class SNbtDeserializer_v1_12 implements ISNbtDeserializer<CompoundTag> {
    ...
    private static final Pattern FLOAT_PATTERN = Pattern.compile(
        "[-+]?(?:[0-9]+[.]?|[0-9]*[.][0-9]+)(?:e[-+]?[0-9]+)?f", 2
    );
    private static final Pattern DOUBLE_PATTERN = Pattern.compile(
        "[-+]?(?:[0-9]+[.]?|[0-9]*[.][0-9]+)(?:e[-+]?[0-9]+)?d", 2
    );
    private static final Pattern SHORT_DOUBLE_PATTERN = Pattern.compile(
        "[-+]?(?:[0-9]+[.]|[0-9]*[.][0-9]+)(?:e[-+]?[0-9]+)?", 2
    );
    ...
    protected Tag readListOrArray(StringReader_v1_12 reader) throws SNbtDeserializeException {
        return reader.canRead(2) && !this.isQuote(reader.charAt(1)) && reader.charAt(2) == ';'
    ...
    protected <T extends NumberTag> ListTag<T> readPrimitiveList(
        StringReader_v1_12 reader, Class<T> primitiveType, Class<? extends Tag> arrayType
        ...
        while (reader.peek() != ']') {
            ...
            if (!primitiveType.isAssignableFrom(tag.getClass())) {
                throw new SNbtDeserializeException(
                    "Unable to insert " + tag.getClass().getSimpleName() + " into " + arrayType.getSimpleName()
                );
            }
    ...
    protected Tag readArray(StringReader_v1_12 reader) throws SNbtDeserializeException {
        ...
        if (c == 'B') {
            ...
            for (int i = 0; i < tags.size(); i++) {
                array[i] = tags.get(i).<Integer>value();
            }
