...
public class HoverEventDeserializer_v1_16 implements JsonDeserializer<AHoverEvent> {
    ...
    public HoverEventDeserializer_v1_16(
        TextComponentSerializer textComponentSerializer, SNbtSerializer<?> sNbtSerializer
    ) {
                ...
                if (contents.isJsonPrimitive()) {
                    ...
                    if (rawItem.has("tag")) {
                        ...
                        return new ItemHoverEvent(
                            action, item, count, (CompoundTag)this.sNbtSerializer.tryDeserialize(rawTag)
                        );
                    }
        ...
        switch (action) {
            ...
            case SHOW_ITEM:
                ...
                Identifier id = Identifier.of(
                    rawTag.get("id") instanceof StringTag ? rawTag.<StringTag>get("id").getValue() : ""
                );
            ...
            case SHOW_ENTITY:
                try {
                    ...
                    ATextComponent name = this.textComponentSerializer
                        .deserialize(
                            rawEntity.get("name") instanceof StringTag
                                ? rawEntity.<StringTag>get("name").getValue()
                                : ""
                        );
                    Identifier entityType = Identifier.of(
                        rawEntity.get("type") instanceof StringTag ? rawEntity.<StringTag>get("type").getValue() : ""
                    );
                    UUID uuid = UUID.fromString(
                        rawEntity.get("id") instanceof StringTag ? rawEntity.<StringTag>get("id").getValue() : ""
                    );
                    return new EntityHoverEvent(action, entityType, uuid, name);
