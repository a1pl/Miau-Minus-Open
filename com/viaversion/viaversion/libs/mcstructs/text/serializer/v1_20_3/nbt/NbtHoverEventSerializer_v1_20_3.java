...
public class NbtHoverEventSerializer_v1_20_3 implements ITypedSerializer<Tag, AHoverEvent>, CodecUtils_v1_20_3 {
    ...
    public NbtHoverEventSerializer_v1_20_3(
        TextComponentCodec codec,
        ITypedSerializer<Tag, ATextComponent> textSerializer,
        SNbtSerializer<CompoundTag> sNbtSerializer
    ) {
        this.codec = codec;
    ...
    public AHoverEvent deserialize(Tag object) {
        ...
        if (action == null) {
            throw new IllegalArgumentException(
                "Unknown hover event action: "
                    + (tag.get("action") instanceof StringTag ? tag.<StringTag>get("action").getValue() : "")
            );
        }
        ...
        if (tag.contains("contents")) {
            switch (action) {
                ...
                case SHOW_ITEM:
                    if (tag.get("contents") instanceof StringTag) {
                        return new ItemHoverEvent(
                            action,
                            Identifier.of(
                                tag.get("contents") instanceof StringTag
                                    ? tag.<StringTag>get("contents").getValue()
                                    : ""
                            ),
                            1,
                            null
                        ...
                        CompoundTag contentsx = tag.get("contents") instanceof CompoundTag
                            ? tag.get("contents")
                            : new CompoundTag();
                        String idx = this.requiredString(contentsx, "id");
                        Integer count = this.optionalInt(contentsx, "count");
                        String itemTag = this.optionalString(contentsx, "tag");
                ...
                case SHOW_ENTITY:
                    ...
                    ATextComponent name = contents.contains("name")
                        ? this.textSerializer.deserialize(contents.get("name"))
                        : null;
                    ...
                    switch (action) {
                        ...
                        case SHOW_ITEM: {
                            ...
                            Identifier id = Identifier.of(
                                parsed.get("id") instanceof StringTag ? parsed.<StringTag>get("id").getValue() : ""
                            );
                            int count = parsed.get("Count") instanceof ByteTag
                                ? parsed.<ByteTag>get("Count").asByte()
                                : 0;
                        ...
                        case SHOW_ENTITY: {
                            ...
                            ATextComponent name = this.codec
                                .deserializeJson(
                                    parsed.get("name") instanceof StringTag
                                        ? parsed.<StringTag>get("name").getValue()
                                        : ""
                                );
                            Identifier type = Identifier.of(
                                parsed.get("type") instanceof StringTag ? parsed.<StringTag>get("type").getValue() : ""
                            );
    ...
    protected UUID getUUID(Tag tag) {
        ...
        return new UUID((long)value[0] << 32 | value[1] & 4294967295L, (long)value[2] << 32 | value[3] & 4294967295L);
    }
}
