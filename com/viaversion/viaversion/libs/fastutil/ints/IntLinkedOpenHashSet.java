...
public class IntLinkedOpenHashSet extends AbstractIntSortedSet implements Serializable, Cloneable, Hash {
    ...
    public static IntLinkedOpenHashSet toSetWithExpectedSize(IntStream stream, int expectedSize) {
        return expectedSize <= 16
            ? toSet(stream)
            : stream.collect(
                new IntCollections.SizeDecreasingSupplier<>(
                    expectedSize, size -> size <= 16 ? new IntLinkedOpenHashSet() : new IntLinkedOpenHashSet(size)
                ),
    ...
    private final class SetIterator implements IntListIterator {
        ...
        SetIterator(int from) {
            if (from == 0) {
                ...
                for (int pos = HashCommon.mix(from) & IntLinkedOpenHashSet.this.mask;
                    key[pos] != 0;
        ...
        public void remove() {
            ...
            if (pos == IntLinkedOpenHashSet.this.n) {
                ...
                while (true) {
                    ...
                    int curr;
                    for (pos = pos + 1 & IntLinkedOpenHashSet.this.mask;
                        (curr = key[pos]) != 0;
                        pos = pos + 1 & IntLinkedOpenHashSet.this.mask
                    ) {
