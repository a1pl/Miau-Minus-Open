...
public final class IntSets {
    ...
    public static IntSet fromTo(int from, int to) {
    ...
    public static IntSet from(int from) {
        return new AbstractIntSet() {
            @Override
            public boolean contains(int x) {
                return x >= from;
            ...
            @Override
            public IntIterator iterator() {
                return IntIterators.concat(
                    IntIterators.fromTo(from, Integer.MAX_VALUE), IntSets.singleton(Integer.MAX_VALUE).iterator()
                );
            }

            @Override
            public int size() {
                long size = 2147483647L - from + 1L;
                return size >= 0L && size <= 2147483647L ? (int)size : Integer.MAX_VALUE;
            }
        };
    }

    public static IntSet to(int to) {
        return new AbstractIntSet() {
            @Override
            public boolean contains(int x) {
                return x < to;
