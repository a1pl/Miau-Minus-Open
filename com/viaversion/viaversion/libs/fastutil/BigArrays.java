...
public class BigArrays {
    ...
    public static void ensureOffsetLength(long bigArrayLength, long offset, long length) {
        ...
        if (length > bigArrayLength - offset) {
            throw new ArrayIndexOutOfBoundsException(
                "Last index ("
                    + Long.toUnsignedString(offset + length)
                    + ") is greater than big-array length ("
                    + bigArrayLength
                    + ")"
            );
        }
            ...
            for (int i = 0; i < l - 1; i++) {
                if (array[i].length != 134217728) {
                    throw new IllegalStateException(
                        "All segments except for the last one must be of length 2^" + Integer.toString(27)
                    );
                }
            }
    ...
    public static void copy(byte[][] srcArray, long srcPos, byte[][] destArray, long destPos, long length) {
        if (destPos <= srcPos) {
            ...
            while (length > 0L) {
                int l = (int)Math.min(
                    length, Math.min(srcArray[srcSegment].length - srcDispl, destArray[destSegment].length - destDispl)
                );
                if (l == 0) {
                    throw new ArrayIndexOutOfBoundsException();
                }
    ...
    public static byte[][] forceCapacity(byte[][] array, long length, long preserve) {
        ...
        int valid = array.length
            - (array.length != 0 && (array.length <= 0 || array[array.length - 1].length != 134217728) ? 1 : 0);
