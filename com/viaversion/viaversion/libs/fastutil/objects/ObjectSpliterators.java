...
public final class ObjectSpliterators {
    ...
    public static <K> ObjectSpliterator<K> wrapPreSorted(
        K[] array, int offset, int length, int additionalCharacteristics, Comparator<? super K> comparator
    ) {
        ObjectArrays.ensureOffsetLength(array, offset, length);
        return new ObjectSpliterators.ArraySpliteratorWithComparator<>(
            array, offset, length, additionalCharacteristics, comparator
        );
    }

    public static <K> ObjectSpliterator<K> wrapPreSorted(
        K[] array, int offset, int length, Comparator<? super K> comparator
    ) {
    ...
    public static <K> ObjectSpliterator<K> asObjectSpliterator(
        Spliterator<K> i, Comparator<? super K> comparatorOverride
    ) {
        if (i instanceof ObjectSpliterator) {
            throw new IllegalArgumentException(
                "Cannot override comparator on instance that is already a " + ObjectSpliterator.class.getSimpleName()
            );
        } else {
    ...
    public static <K> void onEachMatching(
        Spliterator<K> spliterator, Predicate<? super K> predicate, Consumer<? super K> action
    ) {
    ...
    public static <K> ObjectSpliterator<K> asSpliterator(
        ObjectIterator<? extends K> iter, long size, int additionalCharacterisitcs
    ) {
    ...
    public static <K> ObjectSpliterator<K> asSpliteratorFromSorted(
        ...
        return new ObjectSpliterators.SpliteratorFromIteratorWithComparator<>(
            iter, size, additionalCharacterisitcs, comparator
        );
    }

    public static <K> ObjectSpliterator<K> asSpliteratorUnknownSize(
        ObjectIterator<? extends K> iter, int characterisitcs
    ) {
    ...
    public static <K> ObjectSpliterator<K> asSpliteratorFromSortedUnknownSize(
        ...
        return new ObjectSpliterators.SpliteratorFromIteratorWithComparator<>(
            iter, additionalCharacterisitcs, comparator
        );
    }
        ...
        private void splitPointCheck(int splitPoint, int observedMax) {
            if (splitPoint < this.pos || splitPoint > observedMax) {
                throw new IndexOutOfBoundsException(
                    "splitPoint "
                        + splitPoint
                        + " outside of range of current position "
                        + this.pos
                        + " and range end "
                        + observedMax
                );
    ...
    private static class ArraySpliteratorWithComparator<K> extends ObjectSpliterators.ArraySpliterator<K> {
        ...
        public ArraySpliteratorWithComparator(
            K[] array, int offset, int length, int additionalCharacteristics, Comparator<? super K> comparator
        ) {
        ...
        protected ObjectSpliterators.ArraySpliteratorWithComparator<K> makeForSplit(int newOffset, int newLength) {
            return new ObjectSpliterators.ArraySpliteratorWithComparator<>(
                this.array, newOffset, newLength, this.characteristics, this.comparator
            );
        }
    ...
    public abstract static class EarlyBindingSizeIndexBasedSpliterator<K>
    ...
    public abstract static class LateBindingSizeIndexBasedSpliterator<K>
        extends ObjectSpliterators.AbstractIndexBasedSpliterator<K> {
    ...
    private static class SpliteratorFromIterator<K> implements ObjectSpliterator<K> {
        ...
        public ObjectSpliterator<K> trySplit() {
            ...
            int batchSizeEst = this.knownSize && this.size > 0L
                ? (int)Math.min(this.nextBatchSize, this.size)
                : this.nextBatchSize;
    ...
    private static class SpliteratorFromIteratorWithComparator<K> extends ObjectSpliterators.SpliteratorFromIterator<K> {
        ...
        SpliteratorFromIteratorWithComparator(
            ObjectIterator<? extends K> iter, int additionalCharacteristics, Comparator<? super K> comparator
        ) {
    ...
    private static class SpliteratorWrapperWithComparator<K> extends ObjectSpliterators.SpliteratorWrapper<K> {
        ...
        public ObjectSpliterator<K> trySplit() {
            Spliterator<K> innerSplit = this.i.trySplit();
            return innerSplit == null
                ? null
                : new ObjectSpliterators.SpliteratorWrapperWithComparator<>(innerSplit, this.comparator);
        }
    }
}
