...
public interface Object2IntMap<K> extends Object2IntFunction<K>, Map<K, Integer> {
    ...
    default void forEach(BiConsumer<? super K, ? super Integer> consumer) {
        ...
        Consumer<Object2IntMap.Entry<K>> wrappingConsumer = entry -> consumer.accept(
    ...
    default int merge(
        K key, int value, BiFunction<? super Integer, ? super Integer, ? extends Integer> remappingFunction
    ) {
        Objects.requireNonNull(remappingFunction);
        int oldValue = this.getInt(key);
        int drv = this.defaultReturnValue();
    ...
    default int mergeInt(K key, int value, IntBinaryOperator remappingFunction) {
        ...
        int oldValue = this.getInt(key);
        int drv = this.defaultReturnValue();
        int newValue = oldValue == drv && !this.containsKey(key)
            ? value
    ...
    default int mergeInt(
        K key, int value, com.viaversion.viaversion.libs.fastutil.ints.IntBinaryOperator remappingFunction
    ) {
    ...
    @Deprecated
    default Integer merge(
        K key, Integer value, BiFunction<? super Integer, ? super Integer, ? extends Integer> remappingFunction
    ) {
