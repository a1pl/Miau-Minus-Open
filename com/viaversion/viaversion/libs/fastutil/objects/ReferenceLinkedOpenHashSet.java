...
public class ReferenceLinkedOpenHashSet<K>
        ...
        return expectedSize <= 16
            ...
            : Collector.of(
                new ReferenceCollections.SizeDecreasingSupplier<>(
                    expectedSize,
                    size -> size <= 16 ? new ReferenceLinkedOpenHashSet() : new ReferenceLinkedOpenHashSet(size)
                ),
    ...
    public boolean addAndMoveToFirst(K k) {
        ...
        if (k == null) {
            ...
            for (pos = HashCommon.mix(System.identityHashCode(k)) & this.mask;
                key[pos] != null;
                pos = pos + 1 & this.mask
            ) {
                if (k == key[pos]) {
        ...
        public void remove() {
            ...
            if (this.next == -1) {
                ...
                ReferenceLinkedOpenHashSet.this.link[this.next] = ReferenceLinkedOpenHashSet.this.link[this.next]
                    ^ (ReferenceLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 4294967295L) << 32)
                        & -4294967296L;
            }
            ...
            if (pos == ReferenceLinkedOpenHashSet.this.n) {
                ...
                while (true) {
                    ...
                    K curr;
                    for (pos = pos + 1 & ReferenceLinkedOpenHashSet.this.mask;
                        (curr = key[pos]) != null;
                        pos = pos + 1 & ReferenceLinkedOpenHashSet.this.mask
                    ) {
