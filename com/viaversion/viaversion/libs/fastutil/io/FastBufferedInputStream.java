...
public class FastBufferedInputStream extends MeasurableInputStream implements RepositionableStream {
    ...
    public static final EnumSet<FastBufferedInputStream.LineTerminator> ALL_TERMINATORS = EnumSet.allOf(
    ...
    public long skip(long n) throws IOException {
        ...
        while (
            toSkip != 0L
                && (result = this.is == System.in ? this.skipByReading(toSkip) : this.is.skip(toSkip)) < toSkip
        ) {
            if (result == 0L) {
                if (this.is.read() == -1) {
                    break;
