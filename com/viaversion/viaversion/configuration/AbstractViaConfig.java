...
public abstract class AbstractViaConfig extends Config implements ViaVersionConfig {
    ...
    protected void loadFields() {
        ...
        this.blockedDisconnectMessage = this.getString(
        ...
        Map<String, String> worlds = this.get("map-1_16-world-names", new HashMap<>());
    ...
    private BlockedProtocolVersions loadBlockedProtocolVersions() {
        List<Integer> blockProtocols = this.getListSafe(
            "block-protocols", Integer.class, "Invalid blocked version protocol found in config: '%s'"
        );
        List<String> blockVersions = this.getListSafe(
            "block-versions", String.class, "Invalid blocked version found in config: '%s'"
        );
        ObjectSet<ProtocolVersion> blockedProtocols = blockProtocols.stream()
            .map(ProtocolVersion::getProtocol)
            .collect(ObjectOpenHashSet::of, Set::add, Set::addAll);
        ...
        if (lowerBound.isKnown() || upperBound.isKnown()) {
            ...
            blockedProtocols.removeIf(
                version -> {
                    if ((!finalLowerBound.isKnown() || !version.olderThan(finalLowerBound))
                        && (!finalUpperBound.isKnown() || !version.newerThan(finalUpperBound))) {
                        return false;
                    }
