...
public final class Protocol1_20_2To1_20
    ...
    public Protocol1_20_2To1_20() {
        super(
            ClientboundPackets1_19_4.class,
            ClientboundPackets1_20_2.class,
            ServerboundPackets1_19_4.class,
    ...
    protected void registerPackets() {
        ...
        PacketHandlers sanitizeCustomPayload = new PacketHandlers() {
            @Override
        ...
        this.registerClientbound(ClientboundPackets1_19_4.PLUGIN_MESSAGE, sanitizeCustomPayload);
        this.registerServerbound(ServerboundPackets1_20_2.PLUGIN_MESSAGE, sanitizeCustomPayload);
        ...
        this.registerServerbound(
            State.LOGIN, ServerboundLoginPackets.HELLO.getId(), ServerboundLoginPackets.HELLO.getId(), wrapper -> {
                wrapper.passthrough(Type.STRING);
        ...
        this.registerClientbound(
            ...
            wrapper -> {
                wrapper.user()
                    .get(ConfigurationState.class)
                    .setBridgePhase(ConfigurationState.BridgePhase.PROFILE_SENT);
        ...
        this.registerServerbound(
            State.CONFIGURATION, ServerboundConfigurationPackets1_20_2.FINISH_CONFIGURATION.getId(), -1, wrapper -> {
                wrapper.cancel();
        ...
        this.registerServerbound(
            ...
            -1,
            this.queueServerboundPacket(ServerboundPackets1_20_2.KEEP_ALIVE)
        );
        this.registerServerbound(
            State.CONFIGURATION,
            ServerboundConfigurationPackets1_20_2.PONG.getId(),
            -1,
            this.queueServerboundPacket(ServerboundPackets1_20_2.PONG)
        );
        this.registerServerbound(
            State.CONFIGURATION, ServerboundConfigurationPackets1_20_2.RESOURCE_PACK.getId(), -1, PacketWrapper::cancel
        );
        ...
        if (direction == Direction.SERVERBOUND) {
            ...
            if (configurationBridge != null) {
                ...
                if (phase == ConfigurationState.BridgePhase.NONE) {
                    ...
                    if (phase != ConfigurationState.BridgePhase.PROFILE_SENT
                        && phase != ConfigurationState.BridgePhase.REENTERING_CONFIGURATION) {
                        if (packetWrapper.getPacketType() != null
                            && packetWrapper.getPacketType().state() == State.CONFIGURATION) {
                            ...
                            if (!packetWrapper.user().isClientSide()
                                && !Via.getPlatform().isProxy()
                                && unmappedId == ClientboundPackets1_19_4.SYSTEM_CHAT.getId()) {
                                super.transform(direction, State.PLAY, packetWrapper);
                            } else {
                            ...
                            if (unmappedId == ClientboundPackets1_19_4.PLUGIN_MESSAGE.getId()) {
                                ...
                                packetWrapper.setPacketType(
                                    ClientboundConfigurationPackets1_20_2.UPDATE_ENABLED_FEATURES
                                );
                            } else {
    ...
    public static void sendConfigurationPackets(
        UserConnection connection, CompoundTag dimensionRegistry, @Nullable LastResourcePack lastResourcePack
    ) throws Exception {
        ProtocolInfo protocolInfo = connection.getProtocolInfo();
        protocolInfo.setServerState(State.CONFIGURATION);
        PacketWrapper registryDataPacket = PacketWrapper.create(
            ClientboundConfigurationPackets1_20_2.REGISTRY_DATA, connection
        );
        ...
        if (lastResourcePack != null && connection.getProtocolInfo().protocolVersion() == ProtocolVersion.v1_20_2) {
            PacketWrapper resourcePackPacket = PacketWrapper.create(
                ClientboundConfigurationPackets1_20_2.RESOURCE_PACK, connection
            );
        ...
        PacketWrapper finishConfigurationPacket = PacketWrapper.create(
            ClientboundConfigurationPackets1_20_2.FINISH_CONFIGURATION, connection
        );
