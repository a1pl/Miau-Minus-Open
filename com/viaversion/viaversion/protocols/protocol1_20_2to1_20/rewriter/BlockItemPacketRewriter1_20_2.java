...
public final class BlockItemPacketRewriter1_20_2
    ...
    public void registerPackets() {
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_19_4.WINDOW_ITEMS,
                new PacketHandlers() {
                    @Override
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_19_4.SET_SLOT,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.UNSIGNED_BYTE);
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_19_4.ADVANCEMENTS,
                wrapper -> {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        if (wrapper.passthrough(Type.BOOLEAN)) {
                            ...
                            wrapper.passthrough(Type.COMPONENT);
                            wrapper.write(
                                Type.ITEM1_20_2, this.handleItemToClient(wrapper.user(), wrapper.read(Type.ITEM1_13_2))
                            );
                            wrapper.passthrough(Type.VAR_INT);
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_19_4.ENTITY_EQUIPMENT,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.VAR_INT);
        ...
        this.protocol
            .registerServerbound(
                ServerboundPackets1_20_2.CLICK_WINDOW,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.UNSIGNED_BYTE);
        ...
        this.protocol.registerClientbound(ClientboundPackets1_19_4.TRADE_LIST, wrapper -> {
            ...
            for (int i = 0; i < size; i++) {
                wrapper.write(Type.ITEM1_20_2, this.handleItemToClient(wrapper.user(), wrapper.read(Type.ITEM1_13_2)));
                wrapper.write(Type.ITEM1_20_2, this.handleItemToClient(wrapper.user(), wrapper.read(Type.ITEM1_13_2)));
                wrapper.write(Type.ITEM1_20_2, this.handleItemToClient(wrapper.user(), wrapper.read(Type.ITEM1_13_2)));
                wrapper.passthrough(Type.BOOLEAN);
        ...
        this.protocol
            .registerServerbound(
                ServerboundPackets1_20_2.CREATIVE_INVENTORY_ACTION,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.SHORT);
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_19_4.SPAWN_PARTICLE,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.VAR_INT);
            ...
            for (Tag tag : effectsTag) {
                if (tag instanceof CompoundTag) {
                    ...
                    if (idTag instanceof StringTag) {
                        int id = PotionEffects1_20_2.keyToId(((StringTag)idTag).getValue());
    ...
    private @Nullable CompoundTag handleBlockEntity(@Nullable CompoundTag tag) {
        ...
        if (primaryEffect instanceof NumberTag && ((NumberTag)primaryEffect).asInt() != 0) {
            tag.put(
                "primary_effect",
                new StringTag(PotionEffects1_20_2.idToKeyOrLuck(((NumberTag)primaryEffect).asInt() - 1))
            );
        }
        ...
        if (secondaryEffect instanceof NumberTag && ((NumberTag)secondaryEffect).asInt() != 0) {
            tag.put(
                "secondary_effect",
                new StringTag(PotionEffects1_20_2.idToKeyOrLuck(((NumberTag)secondaryEffect).asInt() - 1))
            );
        }
