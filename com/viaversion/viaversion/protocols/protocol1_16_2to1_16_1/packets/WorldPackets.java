...
public class WorldPackets {
    ...
    public static void register(Protocol1_16_2To1_16_1 protocol) {
        ...
        protocol.registerClientbound(
            ClientboundPackets1_16.MULTI_BLOCK_CHANGE,
            wrapper -> {
                wrapper.cancel();
                int chunkX = wrapper.read(Type.INT);
                int chunkZ = wrapper.read(Type.INT);
                long chunkPosition = 0L;
                chunkPosition |= (chunkX & 4194303L) << 42;
                chunkPosition |= (chunkZ & 4194303L) << 20;
                ...
                for (BlockChangeRecord record : blockChangeRecord) {
                    ...
                    list.add(
                        new BlockChangeRecord1_16_2(
                            record.getSectionX(), record.getSectionY(), record.getSectionZ(), blockId
                        )
                    );
                }
                ...
                for (int chunkY = 0; chunkY < sectionRecords.length; chunkY++) {
                    ...
                    if (sectionRecord != null) {
                        ...
                        newPacket.write(Type.LONG, chunkPosition | chunkY & 1048575L);
