...
public class InventoryPackets
    ...
    public void registerPackets() {
                                ...
                                if (wrapper.get(Type.STRING, 0).equals("MC|TrList")) {
                                    ...
                                    for (int i = 0; i < size; i++) {
                                        InventoryPackets.this.handleItemToClient(
                                            wrapper.user(), wrapper.passthrough(Type.ITEM1_8)
                                        );
                                        InventoryPackets.this.handleItemToClient(
                                            wrapper.user(), wrapper.passthrough(Type.ITEM1_8)
                                        );
        ...
        this.protocol
            .registerServerbound(
                ServerboundPackets1_12.CLICK_WINDOW,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        ...
                        this.handler(
                            wrapper -> {
                                ...
                                if (!Via.getConfig().is1_12QuickMoveActionFix()) {
                                    InventoryPackets.this.handleItemToServer(wrapper.user(), item);
                                } else {
                                    ...
                                    if (mode == 1 && button == 0 && item == null) {
                                        ...
                                        InventoryQuickMoveProvider provider = Via.getManager()
                                            .getProviders()
                                            .get(InventoryQuickMoveProvider.class);
                                        boolean succeed = provider.registerQuickMoveAction(
                                            windowId, slotId, actionId, wrapper.user()
                                        );
                                        if (succeed) {
                                            wrapper.cancel();
                                        }
