                    ...
                    protected void register() {
                        this.handler(
                            blockRewriter.chunkDataHandler1_19(
                                ChunkType1_18::new,
                                (user, blockEntity) -> InventoryPackets.this.handleBlockEntity(blockEntity)
                            )
                        );
                        ...
                        this.handler(
                            wrapper -> {
                                for (BlockChangeRecord record : wrapper.passthrough(
                                    ...
                                    record.setBlockId(
                                        InventoryPackets.this.protocol
                                            .getMappingData()
    ...
    private void handleBlockEntity(BlockEntity blockEntity) {
        if (blockEntity.typeId() == 7 || blockEntity.typeId() == 8) {
            ...
            for (int i = 1; i < 5; i++) {
                ...
                messages.add(
                    text instanceof StringTag
                        ? (StringTag)text
                        : new StringTag(ComponentUtil.emptyJsonComponentString())
                );
            }
