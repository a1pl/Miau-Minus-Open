...
public final class EntityPackets extends EntityRewriter<ClientboundPackets1_18, Protocol1_19To1_18_2> {
    ...
    public void registerPackets() {
        ...
        this.registerMetadataRewriter(
            ClientboundPackets1_18.ENTITY_METADATA, Types1_18.METADATA_LIST, Types1_19.METADATA_LIST
        );
        this.registerRemoveEntities(ClientboundPackets1_18.REMOVE_ENTITIES);
        this.protocol
            .registerClientbound(
                ClientboundPackets1_18.SPAWN_ENTITY,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        ...
                        this.handler(
                            wrapper -> {
                                ...
                                EntityType entityType = EntityPackets.this.<EntityTracker>tracker(wrapper.user())
                                    .entityType(entityId);
                                if (entityType == EntityTypes1_19.FALLING_BLOCK) {
                                    wrapper.set(
                                        Type.VAR_INT,
                                        2,
                                        EntityPackets.this.protocol
                                            .getMappingData()
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_18.SPAWN_MOB, ClientboundPackets1_19.SPAWN_ENTITY, new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.VAR_INT);
                        this.map(Type.UUID);
        ...
        this.protocol.registerClientbound(ClientboundPackets1_18.ENTITY_EFFECT, new PacketHandlers() {
            ...
            public void register() {
                ...
                this.map(Type.VAR_INT);
                this.map(Type.BYTE);
                this.create(Type.OPTIONAL_NAMED_COMPOUND_TAG, null);
            }
        });
        this.protocol
            .registerClientbound(
                ClientboundPackets1_18.JOIN_GAME,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.INT);
                        ...
                        this.handler(
                            wrapper -> {
                                ...
                                tag.put(
                                    "minecraft:chat_type", EntityPackets.this.protocol.getMappingData().chatRegistry()
                                );
                                ...
                                for (CompoundTag dimension : dimensions) {
                                    ...
                                    dimensionDataMap.put(
                                        Key.stripMinecraftNamespace(name),
                                        new DimensionDataImpl(idTag.asInt(), element)
                                    );
                                    dimensionsMap.put(element.copy(), name);
                                }

                                EntityPackets.this.<EntityTracker>tracker(wrapper.user())
                                    .setDimensions(dimensionDataMap);
                                DimensionRegistryStorage registryStorage = wrapper.user()
                        ...
                        this.handler(
                            wrapper -> {
                                PacketWrapper displayPreviewPacket = wrapper.create(
                                    ClientboundPackets1_19.SET_DISPLAY_CHAT_PREVIEW
                                );
        ...
        this.protocol
            .registerClientbound(
                ...
                new PacketHandlers() {
                    ...
                    public void register() {
                        this.handler(
                            wrapper -> EntityPackets.writeDimensionKey(
                                wrapper, wrapper.user().get(DimensionRegistryStorage.class)
                            )
                        );
                        this.map(Type.STRING);
                        this.map(Type.LONG);
    ...
    private static void writeDimensionKey(PacketWrapper wrapper, DimensionRegistryStorage registryStorage) throws Exception {
        ...
        if (dimensionKey == null) {
            ...
            dimensionKey = (String)registryStorage.dimensions()
                ...
                .map(
                    it -> new Pair<>(
                        it, Maps.difference(currentDimension.getValue(), it.getKey().getValue()).entriesInCommon()
                    )
                )
                ...
                .orElseThrow(
                    () -> new IllegalArgumentException(
                        "Dimension not found in registry data from join packet: " + currentDimension
                    )
                )
                .key()
                .getValue();
    ...
    protected void registerRewrites() {
        ...
        this.registerMetaTypeHandler(Types1_19.META_TYPES.itemType, Types1_19.META_TYPES.blockStateType, null);
