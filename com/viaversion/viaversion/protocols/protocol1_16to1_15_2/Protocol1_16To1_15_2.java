...
public class Protocol1_16To1_15_2
    ...
    public Protocol1_16To1_15_2() {
        super(
            ClientboundPackets1_15.class,
            ClientboundPackets1_16.class,
            ServerboundPackets1_14.class,
            ServerboundPackets1_16.class
        );
    }
    ...
    protected void registerPackets() {
        ...
        this.registerClientbound(
            ClientboundPackets1_15.CHAT_MESSAGE,
            new PacketHandlers() {
                @Override
                public void register() {
                    ...
                    this.handler(
                        wrapper -> {
                            Protocol1_16To1_15_2.this.componentRewriter
                                .processText(wrapper.user(), wrapper.get(Type.COMPONENT, 0));
                                ...
                                if (channel.length() > 32) {
                                    if (!Via.getConfig().isSuppressConversionWarnings()) {
                                        Via.getPlatform()
                                            .getLogger()
                                            .warning(
                                                "Ignoring serverbound plugin channel, as it is longer than 32 characters: "
                                                    + channel
                                            );
                                    }

                                    wrapper.cancel();
                                } else if (namespacedChannel.equals("minecraft:register")
                                    || namespacedChannel.equals("minecraft:unregister")) {
                                        ...
                                        if (registeredChannel.length() > 32) {
                                            if (!Via.getConfig().isSuppressConversionWarnings()) {
                                                Via.getPlatform()
                                                    .getLogger()
                                                    .warning(
                                                        "Ignoring serverbound plugin channel register of '"
                                                            + registeredChannel
                                                            + "', as it is longer than 32 characters"
                                                    );
                                            }
                                        } else {
                                    ...
                                    wrapper.write(
                                        ...
                                        Joiner.on('\u0000').join(checkedChannels).getBytes(StandardCharsets.UTF_8)
                                    );
                                }
                            }
                        );
        ...
        this.registerServerbound(
            ServerboundPackets1_16.PLAYER_ABILITIES,
            wrapper -> {
                wrapper.passthrough(Type.BYTE);
                PlayerAbilitiesProvider playerAbilities = Via.getManager()
                    .getProviders()
                    .get(PlayerAbilitiesProvider.class);
    ...
    public void init(UserConnection userConnection) {
        userConnection.addEntityTracker(
            (Class<? extends Protocol>)this.getClass(), new EntityTrackerBase(userConnection, EntityTypes1_16.PLAYER)
        );
