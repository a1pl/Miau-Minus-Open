    ...
    public static void register(Protocol1_9To1_8 protocol) {
        protocol.registerClientbound(ClientboundPackets1_8.CHAT_MESSAGE, new PacketHandlers() {
            ...
            public void register() {
                this.map(Type.STRING, Protocol1_9To1_8.STRING_TO_JSON);
                this.map(Type.BYTE);
                                ...
                                if (action != 0) {
                                    if (action == 1 || action == 2) {
                                        ...
                                        wrapper.write(
                                            ...
                                            displayName != null
                                                ? Protocol1_9To1_8.STRING_TO_JSON.transform(wrapper, displayName)
                                                : null
                                        );
                                    }
                                } else {
        ...
        protocol.registerServerbound(
            ServerboundPackets1_9.CLIENT_SETTINGS,
            new PacketHandlers() {
                @Override
                public void register() {
                    this.map(Type.STRING);
                    ...
                    this.handler(
                        wrapper -> {
                            ...
                            if (Via.getConfig().isLeftHandedHandling() && hand == 0) {
                                wrapper.set(
                                    Type.UNSIGNED_BYTE, 0, (short)(wrapper.get(Type.UNSIGNED_BYTE, 0).intValue() | 128)
                                );
                            }
                            ...
                            Via.getManager()
                                .getProviders()
                                .get(MainHandProvider.class)
                                .setMainHand(wrapper.user(), hand);
                        }
                    );
                }
            }
