...
public class SpawnPackets {
    public static final ValueTransformer<Integer, Double> toNewDouble = new ValueTransformer<Integer, Double>(
        Type.DOUBLE
    ) {
        public Double transform(PacketWrapper wrapper, Integer inputValue) {
            return inputValue.intValue() / 32.0;
        }
    };
                    ...
                    this.handler(
                        wrapper -> {
                            ...
                            if (tracker.hasEntity(entityId)) {
                                ...
                                Via.getPlatform()
                                    .getLogger()
                                    .warning("Unable to find entity for metadata, entity ID: " + entityId);
                                metadataList.clear();
                            }
                        }
                    );
                        ...
                        wrapper -> {
                            ...
                            if (item != 0) {
                                PacketWrapper packet = PacketWrapper.create(
                                    ClientboundPackets1_9.ENTITY_EQUIPMENT, null, wrapper.user()
                                );
                                ...
                                try {
                                    ...
                                    Via.getPlatform()
                                        .getLogger()
                                        .log(Level.SEVERE, "Failed to send entity equipment packet", e);
                                }
                            }
                        }
                    );
