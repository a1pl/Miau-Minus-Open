...
public class WorldPackets {
    public static void register(Protocol1_9To1_8 protocol) {
        ...
        protocol.registerClientbound(
            ClientboundPackets1_8.NAMED_SOUND,
            new PacketHandlers() {
                @Override
                public void register() {
                    ...
                    this.handler(
                        wrapper -> {
                            ...
                            if (effect != null && effect.isBreakSound()) {
                                ...
                                if (tracker.interactedBlockRecently(
                                    (int)Math.floor(x / 8.0), (int)Math.floor(y / 8.0), (int)Math.floor(z / 8.0)
                                )) {
                                    wrapper.cancel();
                                }
                            }
        ...
        protocol.registerClientbound(
            ClientboundPackets1_8.MAP_BULK_CHUNK,
            null,
            wrapper -> {
                wrapper.cancel();
                ClientWorld clientWorld = wrapper.user().get(ClientWorld.class);
        ...
        protocol.registerClientbound(
            ClientboundPackets1_8.BLOCK_ENTITY_DATA,
            new PacketHandlers() {
                @Override
                public void register() {
                    this.map(Type.POSITION1_8);
                    ...
                    this.handler(
                        wrapper -> {
                            ...
                            if (action == 2) {
                                CommandBlockProvider provider = Via.getManager()
                                    .getProviders()
                                    .get(CommandBlockProvider.class);
                                provider.addOrUpdateBlock(
                                    wrapper.user(),
        ...
        protocol.registerServerbound(
            ServerboundPackets1_9.USE_ITEM,
            null,
            wrapper -> {
                ...
                if (Via.getConfig().isShieldBlocking()) {
                    ...
                    boolean isSword = showShieldWhenSwordInHand
                        ? tracker.hasSwordInHand()
        ...
        protocol.registerServerbound(
            ServerboundPackets1_9.PLAYER_BLOCK_PLACEMENT,
            new PacketHandlers() {
                @Override
                public void register() {
                    this.map(Type.POSITION1_8);
                    ...
                    this.handler(
                        wrapper -> {
                            ...
                            if (tag.isPresent()) {
                                PacketWrapper updateBlockEntity = PacketWrapper.create(
                                    ClientboundPackets1_9.BLOCK_ENTITY_DATA, null, wrapper.user()
                                );
