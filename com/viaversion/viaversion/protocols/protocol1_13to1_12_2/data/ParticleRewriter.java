...
public class ParticleRewriter {
    ...
    public static Particle rewriteParticle(int particleId, Integer[] data) {
        if (particleId >= particles.size()) {
            Via.getPlatform()
                .getLogger()
                .severe("Failed to transform particles with id " + particleId + " and data " + Arrays.toString(data));
            return null;
        } else {
    ...
    private static class NewParticle {
        ...
        public ParticleRewriter.ParticleDataHandler handler() {
            return this.handler;
        }
    }
