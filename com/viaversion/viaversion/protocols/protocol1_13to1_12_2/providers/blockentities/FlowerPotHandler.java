...
public class FlowerPotHandler implements BlockEntityProvider.BlockEntityHandler {
    ...
    public int transform(UserConnection user, CompoundTag tag) {
        ...
        if (item instanceof String) {
            var6 = Key.stripMinecraftNamespace((String)item);
        } else if (item instanceof Number) {
            var6 = ((Number)item).byteValue();
        } else {
            var6 = (byte)0;
        }
        ...
        Integer flower = flowers.get(new Pair<>(var6, (Byte)data));
        if (flower != null) {
            return flower;
        }

        flower = flowers.get(new Pair<>(var6, (byte)0));
