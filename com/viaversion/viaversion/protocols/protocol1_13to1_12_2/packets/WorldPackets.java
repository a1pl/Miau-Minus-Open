...
public class WorldPackets {
    ...
    public static void register(Protocol1_13To1_12_2 protocol) {
        protocol.registerClientbound(
            ClientboundPackets1_12_1.SPAWN_PAINTING,
            new PacketHandlers() {
                @Override
                public void register() {
                    ...
                    this.handler(
                        wrapper -> {
                            ...
                            if (!id.isPresent()
                                && (!Via.getConfig().isSuppressConversionWarnings() || Via.getManager().isDebug())) {
                                Via.getPlatform()
                                    .getLogger()
                                    .warning(
                                        "Could not find painting motive: " + motive + " falling back to default (0)"
                                    );
                            }
        ...
        protocol.registerClientbound(
            ClientboundPackets1_12_1.BLOCK_CHANGE,
            new PacketHandlers() {
                @Override
                public void register() {
                    this.map(Type.POSITION1_8);
                    ...
                    this.handler(
                        wrapper -> {
                            ...
                            if (Via.getConfig().isServersideBlockConnections()) {
                                ...
                                ConnectionData.updateBlockStorage(
                                    userConnection, position.x(), position.y(), position.z(), newId
                                );
                            }
        ...
        protocol.registerClientbound(
            ClientboundPackets1_12_1.MULTI_BLOCK_CHANGE,
            new PacketHandlers() {
                @Override
                public void register() {
                    this.map(Type.INT);
                    ...
                    this.handler(
                        wrapper -> {
                            ...
                            for (BlockChangeRecord record : records) {
                                ...
                                if (Via.getConfig().isServersideBlockConnections()) {
                                    ConnectionData.updateBlockStorage(
                                        userConnection, position.x(), position.y(), position.z(), newBlock
                                    );
                                }
                            }
                            ...
                            if (Via.getConfig().isServersideBlockConnections()) {
                                for (BlockChangeRecord record : records) {
                                    ...
                                    if (handler != null) {
                                        ...
                                        ConnectionData.updateBlockStorage(
                                            userConnection, position.x(), position.y(), position.z(), blockState
                                        );
                                    }
                                }
                        ...
                        this.handler(
                            wrapper -> {
                                ...
                                for (int i = 0; i < recordCount; i++) {
                                    ...
                                    records[i] = position;
                                    ConnectionData.updateBlockStorage(
                                        userConnection, position.x(), position.y(), position.z(), 0
                                    );
                                }
                        ...
                        if (Via.getConfig().isServersideBlockConnections() && ConnectionData.needStoreBlocks()) {
                            if (!chunk.isFullChunk()) {
                                ConnectionData.blockConnectionProvider
                                    .unloadChunkSection(wrapper.user(), chunk.getX(), s, chunk.getZ());
                            }
                            ...
                            if (willSave) {
                                for (int idx = 0; idx < 4096; idx++) {
                                    ...
                                    if (ConnectionData.isWelcome(id)) {
                                        ...
                                        ConnectionData.blockConnectionProvider
                                            .storeBlock(wrapper.user(), globalX, globalY, globalZ, id);
                                    }
                                }
                            }
                        }
    ...
    public static int toNewId(int oldId) {
        ...
        newId = Protocol1_13To1_12_2.MAPPINGS.getBlockMappings().getNewId(IdAndData.removeData(oldId));
        if (newId != -1) {
            if (!Via.getConfig().isSuppressConversionWarnings() || Via.getManager().isDebug()) {
