...
public class RedstoneConnectionHandler extends ConnectionHandler {
    ...
    private int connects(UserConnection user, Position position, BlockFace side) {
        ...
        if (REDSTONE.contains(up)
            && !ConnectionData.OCCLUDING_STATES.contains(this.getBlockData(user, position.getRelative(BlockFace.TOP)))) {
            return 2;
        }

        int down = this.getBlockData(user, relative.getRelative(BlockFace.BOTTOM));
        return REDSTONE.contains(down) && !ConnectionData.OCCLUDING_STATES.contains(this.getBlockData(user, relative))
            ? 1
            : 0;
    }
