...
public class StairConnectionHandler extends ConnectionHandler {
    ...
    private int getShape(UserConnection user, Position position, StairConnectionHandler.StairData stair) {
        ...
        StairConnectionHandler.StairData relativeStair = STAIR_DATA_MAP.get(
            this.getBlockData(user, position.getRelative(facing))
        );
    ...
    private boolean checkOpposite(
        UserConnection user, StairConnectionHandler.StairData stair, Position position, BlockFace face
    ) {
        StairConnectionHandler.StairData relativeStair = STAIR_DATA_MAP.get(
            this.getBlockData(user, position.getRelative(face))
        );
        return relativeStair == null
            || relativeStair.getFacing() != stair.getFacing()
            || relativeStair.isBottom() != stair.isBottom();
    }
