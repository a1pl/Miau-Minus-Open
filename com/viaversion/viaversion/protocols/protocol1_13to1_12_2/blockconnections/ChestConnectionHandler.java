...
class ChestConnectionHandler extends ConnectionHandler {
    ...
    static ConnectionData.ConnectorInitAction init() {
        ...
        return blockData -> {
            if (blockData.getMinecraftKey().equals("minecraft:chest")
                || blockData.getMinecraftKey().equals("minecraft:trapped_chest")) {
                if (!blockData.getValue("waterlogged").equals("true")) {
                    CHEST_FACINGS.put(
                        blockData.getSavedBlockStateId(),
    ...
    public int connect(UserConnection user, Position position, int blockState) {
        ...
        int relative;
        if (CHEST_FACINGS.containsKey(relative = this.getBlockData(user, position.getRelative(BlockFace.NORTH)))
