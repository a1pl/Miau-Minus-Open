...
public class SnowyGrassConnectionHandler extends ConnectionHandler {
    ...
    static ConnectionData.ConnectorInitAction init() {
        ...
        return blockData -> {
            if (snowyGrassBlocks.contains(blockData.getMinecraftKey())) {
                ...
                GRASS_BLOCKS.put(
                    new SnowyGrassConnectionHandler.GrassBlock(blockData.getSavedBlockStateId(), true),
                    blockData.getBlockStateId()
                );
                blockData.set("snowy", "false");
                GRASS_BLOCKS.put(
                    new SnowyGrassConnectionHandler.GrassBlock(blockData.getSavedBlockStateId(), false),
                    blockData.getBlockStateId()
                );
            }

            if (blockData.getMinecraftKey().equals("minecraft:snow")
                || blockData.getMinecraftKey().equals("minecraft:snow_block")) {
                ConnectionData.connectionHandlerMap.put(blockData.getSavedBlockStateId(), handler);
    ...
    public int connect(UserConnection user, Position position, int blockState) {
        ...
        int newId = GRASS_BLOCKS.getInt(
            new SnowyGrassConnectionHandler.GrassBlock(blockState, SNOWY_GRASS_BLOCKS.contains(blockUpId))
        );
