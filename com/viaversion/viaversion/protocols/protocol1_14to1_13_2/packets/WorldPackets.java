...
public class WorldPackets {
    ...
    public static void register(Protocol1_14To1_13_2 protocol) {
        ...
        protocol.registerClientbound(
            ClientboundPackets1_13.BLOCK_ACTION,
            new PacketHandlers() {
                @Override
                public void register() {
                    this.map(Type.POSITION1_8, Type.POSITION1_14);
                    ...
                    this.handler(
                        wrapper -> wrapper.set(
                            Type.VAR_INT, 0, protocol.getMappingData().getNewBlockId(wrapper.get(Type.VAR_INT, 0))
                        )
                    );
                }
            }
                            ...
                            for (int idx = 0; idx < 4096; idx++) {
                                ...
                                if (id != air && id != voidAir && id != caveAir) {
                                    ...
                                    if (Via.getConfig().isNonFullBlockLightFix()
                                        && protocol.getMappingData().getNonFullBlocks().contains(id)) {
    ...
    private static long getChunkIndex(int x, int z) {
        return (x & 67108863L) << 38 | z & 67108863L;
    }
