...
public class ViaManagerImpl implements ViaManager {
    ...
    public ViaManagerImpl(
        ViaPlatform<?> platform, ViaInjector injector, ViaCommandHandler commandHandler, ViaPlatformLoader loader
    ) {
    ...
    public void onServerLoaded() {
        ...
        if (protocolVersion.isKnown()) {
            if (this.platform.isProxy()) {
                this.platform
                    .getLogger()
                    .info(
                        "ViaVersion detected lowest supported version by the proxy: "
                            + protocolVersion.lowestSupportedProtocolVersion()
                    );
                this.platform
                    .getLogger()
                    .info(
                        "Highest supported version by the proxy: " + protocolVersion.highestSupportedProtocolVersion()
                    );
                    ...
                    this.platform
                        ...
                        .info(
                            "Supported version range: "
                                + Arrays.toString(
                                    protocolVersion.supportedProtocolVersions().toArray(new ProtocolVersion[0])
                                )
                        );
                }
            } else {
                this.platform
                    .getLogger()
                    .info("ViaVersion detected server version: " + protocolVersion.highestSupportedProtocolVersion());
            }

            if (!this.protocolManager.isWorkingPipe()) {
                this.platform
                    .getLogger()
                    .warning("ViaVersion does not have any compatible versions for this server version!");
                this.platform
                    .getLogger()
                    .warning(
                        "Please remember that ViaVersion only adds support for versions newer than the server version."
                    );
                this.platform
                    .getLogger()
                    .warning(
                        "If you need support for older versions you may need to use one or more ViaVersion addons too."
                    );
                this.platform
                    .getLogger()
                    .warning(
                        "In that case please read the ViaVersion resource page carefully or use https://viaversion.com/setup"
                    );
                this.platform
                    .getLogger()
                    .warning("and if you're still unsure, feel free to join our Discord-Server for further assistance.");
        ...
        ProtocolVersion serverProtocolVersion = this.protocolManager
    ...
    private void loadServerProtocol() {
        try {
            ...
            if (this.platform.isProxy()) {
                ...
                versionInfo = new ServerProtocolVersionRange(
                    supportedVersions.first(), supportedVersions.last(), supportedVersions
                );
            } else {
    ...
    public void destroy() {
        this.platform
            .getLogger()
            .info("ViaVersion is disabling, if this is a reload and you experience issues consider rebooting.");
    ...
    private void checkJavaVersion() {
        ...
        if (!matcher.find()) {
            ...
            try {
                ...
                this.platform
                    .getLogger()
                    .log(Level.WARNING, "Failed to determine Java version; could not parse: " + versionString, e);
                return;
            }
            ...
            if (version < 17) {
                ...
                this.platform
                    .getLogger()
                    .warning(
                        "Starting with Minecraft 1.21, ViaVersion will no longer officially be compatible with this version of Java, only offering unsupported compatibility builds."
                    );
            }
        }
    }
    ...
    private void unsupportedSoftwareWarning() {
        ...
        for (UnsupportedSoftware software : this.platform.getUnsupportedSoftwareClasses()) {
            ...
            if (match != null) {
                if (!found) {
                    ...
                    this.platform
                        .getLogger()
                        .severe("You are using unsupported software and may encounter unforeseeable issues.");
                    this.platform.getLogger().severe("");
                    found = true;
                }
        ...
        if (found) {
            this.platform
                .getLogger()
                .severe("We will not provide support in case you encounter issues possibly related to this software.");
            this.platform.getLogger().severe("************************************************");
        }
    }
