...
public class UserConnectionImpl implements UserConnection {
    ...
    private final Set<UUID> passthroughTokens = Collections.newSetFromMap(
        CacheBuilder.newBuilder().expireAfterWrite(10L, TimeUnit.SECONDS).build().asMap()
    );
    ...
    private void sendRawPacket(ByteBuf packet, boolean currentThread) {
        ...
        if (this.clientSide) {
            act = () -> this.getChannel()
                .pipeline()
                .context(Via.getManager().getInjector().getDecoderName())
                .fireChannelRead(packet);
        } else {
            act = () -> this.channel
                .pipeline()
                .context(Via.getManager().getInjector().getEncoderName())
    ...
    private void sendRawPacketToServerServerSide(ByteBuf packet, boolean currentThread) {
        ...
        try {
            ChannelHandlerContext context = PipelineUtil.getPreviousContext(
                Via.getManager().getInjector().getDecoderName(), this.channel.pipeline()
            );
    ...
    public boolean checkServerboundPacket() {
        return this.pendingDisconnect
            ? false
            : !this.packetLimiterEnabled
                || !this.packetTracker.incrementReceived()
                || !this.packetTracker.exceedsMaxPPS();
    }
