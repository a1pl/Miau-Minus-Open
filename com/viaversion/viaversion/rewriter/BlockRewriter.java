...
public class BlockRewriter<C extends ClientboundPacketType> {
                    ...
                    public void register() {
                        ...
                        this.handler(
                            wrapper -> {
                                ...
                                if (id == playRecordId
                                    && BlockRewriter.this.protocol.getMappingData().getItemMappings() != null) {
                                    wrapper.set(
                                        Type.INT, 1, BlockRewriter.this.protocol.getMappingData().getNewItemId(data)
                                    );
                                } else if (id == blockBreakId
                                    && BlockRewriter.this.protocol.getMappingData().getBlockStateMappings() != null) {
                                    wrapper.set(
                                        Type.INT,
                                        1,
                                        BlockRewriter.this.protocol.getMappingData().getNewBlockStateId(data)
                                    );
                                }
                            }
                        );
    ...
    public void registerChunkData1_19(
        C packetType,
        BlockRewriter.ChunkTypeSupplier chunkTypeSupplier,
        @Nullable BiConsumer<UserConnection, BlockEntity> blockEntityHandler
    ) {
        this.protocol.registerClientbound(packetType, this.chunkDataHandler1_19(chunkTypeSupplier, blockEntityHandler));
    }

    public PacketHandler chunkDataHandler1_19(
        BlockRewriter.ChunkTypeSupplier chunkTypeSupplier,
        @Nullable BiConsumer<UserConnection, BlockEntity> blockEntityHandler
    ) {
        return wrapper -> {
            ...
            if (blockEntityMappings != null || blockEntityHandler != null) {
                ...
                for (int i = 0; i < blockEntities.size(); i++) {
                    ...
                    if (blockEntityMappings != null) {
                        blockEntities.set(
                            i,
                            blockEntity.withTypeId(
                                blockEntityMappings.getNewIdOrDefault(blockEntity.typeId(), blockEntity.typeId())
                            )
                        );
                    }
            ...
            .registerClientbound(
                ...
                wrapper -> {
                    ...
                    if (blockEntityHandler != null && (tag = wrapper.passthrough(this.compoundTagType)) != null) {
                        BlockEntity blockEntity = new BlockEntityImpl(
                            BlockEntity.pack(position.x(), position.z()), (short)position.y(), blockEntityId, tag
                        );
