...
public abstract class LegacyViaInjector implements ViaInjector {
    ...
    public void inject() throws ReflectiveOperationException {
        ...
        for (Field field : connection.getClass().getDeclaredFields()) {
            if (List.class.isAssignableFrom(field.getType())
                && field.getGenericType().getTypeName().contains(ChannelFuture.class.getName())) {
    ...
    private void injectChannelFuture(ChannelFuture future) throws ReflectiveOperationException {
        ...
        try {
            ChannelInitializer<Channel> oldInitializer = ReflectionUtil.get(
                bootstrapAcceptor, "childHandler", ChannelInitializer.class
            );
    ...
    public void uninject() throws ReflectiveOperationException {
            ...
            if (bootstrapAcceptor == null) {
                ...
                while (true) {
                    if (originalList.hasNext()) {
                        ...
                        try {
                            if (!(
                                ReflectionUtil.get(handler, "childHandler", ChannelInitializer.class) instanceof WrappedChannelInitializer
                            )) {
                                continue;
                            }
                    ...
                    try {
                        ChannelInitializer<Channel> initializer = ReflectionUtil.get(
                            bootstrapAcceptor, "childHandler", ChannelInitializer.class
                        );
                        if (initializer instanceof WrappedChannelInitializer) {
                            ReflectionUtil.set(
                                bootstrapAcceptor, "childHandler", ((WrappedChannelInitializer)initializer).original()
                            );
                        }
                    } catch (Exception e) {
                        Via.getPlatform()
        ...
        for (Pair<Field, Object> pair : this.injectedLists) {
            try {
                ...
                Via.getPlatform()
                    .getLogger()
                    .severe("Failed to remove injection, reload won't work with connections, please reboot!");
            }
        }
                ...
                if (channelHandler == null) {
                    ...
                    try {
                        ...
                        if (child instanceof WrappedChannelInitializer) {
                            handlerInfo.addProperty(
                                "oldInit", ((WrappedChannelInitializer)child).original().getClass().getName()
                            );
                        }
        ...
        try {
            for (Pair<Field, Object> pair : this.injectedLists) {
                ...
                if (list instanceof SynchronizedListWrapper) {
                    wrappedLists.addProperty(
                        field.getName(), ((SynchronizedListWrapper)list).originalList().getClass().getName()
                    );
                }
            }
