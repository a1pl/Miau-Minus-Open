...
public class PacketWrapperImpl implements PacketWrapper {
    ...
    public PacketWrapperImpl(
        @Nullable PacketType packetType, @Nullable ByteBuf inputBuffer, UserConnection userConnection
    ) {
    ...
    public <T> T get(Type<T> type, int index) throws Exception {
        ...
        throw this.createInformativeException(
            new ArrayIndexOutOfBoundsException("Could not find type " + type.getTypeName() + " at " + index),
            type,
            index
        );
    }
    ...
    public <T> T read(Type<T> type) throws Exception {
        if (this.readableObjects.isEmpty()) {
            ...
            if (readType != type
                ...
                throw this.createInformativeException(
                    new IOException(
                        "Unable to read type " + type.getTypeName() + ", found " + readValue.type().getTypeName()
                    ),
                    type,
                    this.readableObjects.size()
                );
            } else {
    ...
    private <T> @Nullable T attemptTransform(Type<T> expectedType, @Nullable T value) {
        if (value != null && !expectedType.getOutputClass().isAssignableFrom(value.getClass())) {
            ...
            Via.getPlatform()
                .getLogger()
                .warning(
                    "Possible type mismatch: " + value.getClass().getName() + " -> " + expectedType.getOutputClass()
                );
        }
    ...
    public void writeToBuffer(ByteBuf buffer) throws Exception {
        ...
        for (int i = 0; i < this.packetValues.size(); i++) {
            PacketWrapperImpl.PacketValue<?> packetValue = this.packetValues.get(i);
            ...
            if (currentThread) {
                ...
                connection.getChannel().eventLoop().submit(() -> {
                    try {
                        ...
                        if (!PipelineUtil.containsCause(exx, CancelException.class)) {
                            throw exx;
                        }
                    } catch (Exception e) {
                        if (!PipelineUtil.containsCause(ex, CancelException.class)) {
    ...
    private ByteBuf constructPacket(
        ...
        ByteBuf output = this.inputBuffer == null
            ? this.user().getChannel().alloc().buffer()
            : this.inputBuffer.alloc().buffer();
    ...
    public ChannelFuture sendFuture(Class<? extends Protocol> protocolClass) throws Exception {
        if (!this.isCancelled()) {
            ByteBuf output = this.constructPacket(protocolClass, true, Direction.CLIENTBOUND);
    ...
    @Deprecated
    public PacketWrapperImpl apply(
        Direction direction, State state, int index, List<Protocol> pipeline, boolean reverse
    ) throws Exception {
        if (reverse) {
