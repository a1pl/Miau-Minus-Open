...
public class DebugSubCmd extends ViaSubCommand {
    ...
    public boolean execute(ViaCommandSender sender, String[] args) {
        ...
        if (args.length == 0) {
            ...
            sendMessage(
                sender,
                "&6Debug mode is now %s",
                Via.getManager().debugHandler().enabled() ? "&aenabled" : "&cdisabled"
            );
            return true;
        }
        ...
        if (args.length == 1) {
            ...
            if (args[0].equalsIgnoreCase("logposttransform")) {
                ...
                sendMessage(
                    sender,
                    "&6Post transform packet logging is now %s",
                    debug.logPostPacketTransform() ? "&aenabled" : "&cdisabled"
                );
                return true;
            }
