...
public class BlockItemPackets1_13
    extends BackwardsItemRewriter<ClientboundPackets1_13, ServerboundPackets1_12_1, Protocol1_12_2To1_13> {
    ...
    protected void registerPackets() {
        ...
        this.protocol.registerClientbound(ClientboundPackets1_13.BLOCK_ACTION, new PacketHandlers() {
            ...
            public void register() {
                ...
                this.handler(wrapper -> {
                    ...
                    if (blockId == 25) {
                        NoteBlockStorage noteBlockStorage = wrapper.user().get(NoteBlockStorage.class);
            ...
            .registerClientbound(
                ...
                new PacketHandlers() {
                    ...
                    public void register() {
                        ...
                        this.handler(
                            wrapper -> {
                                BackwardsBlockEntityProvider provider = Via.getManager()
                                    .getProviders()
                                    .get(BackwardsBlockEntityProvider.class);
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_13.UNLOAD_CHUNK,
                wrapper -> {
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_13.BLOCK_CHANGE,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.POSITION1_8);
                        this.handler(
                            wrapper -> {
                                ...
                                if (blockState >= 249 && blockState <= 748) {
                                    wrapper.user()
                                        .get(NoteBlockStorage.class)
                                        .storeNoteBlockUpdate(position, blockState);
                                }
                                ...
                                wrapper.write(
                                    Type.VAR_INT,
                                    BlockItemPackets1_13.this.protocol.getMappingData().getNewBlockStateId(blockState)
                                );
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_13.MULTI_BLOCK_CHANGE,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.INT);
                        ...
                        this.handler(
                            wrapper -> {
                                ...
                                for (BlockChangeRecord record : wrapper.get(Type.BLOCK_CHANGE_RECORD_ARRAY, 0)) {
                                    ...
                                    record.setBlockId(
                                        BlockItemPackets1_13.this.protocol.getMappingData().getNewBlockStateId(block)
                                    );
                                }
                            }
                        );
        ...
        this.protocol.registerClientbound(ClientboundPackets1_13.WINDOW_ITEMS, new PacketHandlers() {
            ...
            public void register() {
                ...
                this.handler(wrapper -> {
                    Item[] items = wrapper.get(Type.ITEM1_8_SHORT_ARRAY, 0);

                    for (Item item : items) {
                        BlockItemPackets1_13.this.handleItemToClient(wrapper.user(), item);
                    }
                });
            }
        });
        this.protocol
            .registerClientbound(
                ClientboundPackets1_13.SET_SLOT,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.UNSIGNED_BYTE);
                        ...
                        this.handler(
                            wrapper -> BlockItemPackets1_13.this.handleItemToClient(
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_13.CHUNK_DATA,
                wrapper -> {
                    ...
                    for (int i = 0; i < chunk.getSections().length; i++) {
                        ...
                        if (section != null) {
                                    ...
                                    for (int x = 0; x < 16; x++) {
                                        ...
                                        if (FlowerPotHandler.isFlowah(block)) {
                                            Position pos = new Position(
                                                x + (chunk.getX() << 4), (short)(y + (i << 4)), z + (chunk.getZ() << 4)
                                            );
                                            storage.checkAndStore(pos, block);
                                            CompoundTag nbt = provider.transform(
                                                wrapper.user(), pos, "minecraft:flower_pot"
                                            );
                            ...
                            for (int j = 0; j < palette.size(); j++) {
                                int mappedBlockStateId = this.protocol
                                    .getMappingData()
                                    .getNewBlockStateId(palette.idByIndex(j));
                            ...
                            wrapper -> {
                                ...
                                if (id == 1010) {
                                    wrapper.set(
                                        Type.INT,
                                        1,
                                        BlockItemPackets1_13.this.protocol
                                                .getMappingData()
                                                .getItemMappings()
                                                .getNewId(data)
                                            >> 4
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_13.ENTITY_EQUIPMENT,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.VAR_INT);
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_13.WINDOW_PROPERTY,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.UNSIGNED_BYTE);
                            ...
                            wrapper -> {
                                ...
                                if (property >= 4 && property <= 6) {
                                    ...
                                    wrapper.set(
                                        Type.SHORT,
                                        1,
                                        (short)BlockItemPackets1_13.this.protocol
                                            .getMappingData()
        ...
        this.protocol
            .registerServerbound(
                ServerboundPackets1_12_1.CREATIVE_INVENTORY_ACTION,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.SHORT);
                        ...
                        this.handler(
                            wrapper -> BlockItemPackets1_13.this.handleItemToServer(
        ...
        this.protocol
            .registerServerbound(
                ServerboundPackets1_12_1.CLICK_WINDOW,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.UNSIGNED_BYTE);
        ...
        if (tag != null) {
            ...
            if (display != null) {
                ...
                if (name != null) {
                    ...
                    name.setValue(this.protocol.jsonToLegacy(connection, name.getValue()));
                }
            }
        ...
        if (blockTag != null) {
            ...
            for (Tag oldTag : blockTag) {
                ...
                String[] newValues = value instanceof String
                    ? BlockIdData.fallbackReverseMapping.get(Key.stripMinecraftNamespace((String)value))
                    : null;
                if (newValues != null) {
        ...
        if (enchantments != null) {
                ...
                if (idTag != null) {
                    ...
                    if (levelTag != null) {
                        ...
                        if (mappedEnchantmentId != null) {
                            lore.add(
                                new StringTag(mappedEnchantmentId + " " + EnchantmentRewriter.getRomanNumber(level))
                            );
                            noMapped.add(enchantmentEntry);
                        } else if (!newId.isEmpty()) {
                            Short oldId = (Short)Protocol1_13To1_12_2.MAPPINGS
                                .getOldEnchantmentsIds()
                                .inverse()
                                .get(Key.stripMinecraftNamespace(newId));
                            if (oldId == null) {
                                if (!newId.startsWith("viaversion:legacy/")) {
                                    ...
                                    if (Via.getManager().isDebug()) {
                                        ViaBackwards.getPlatform()
                                            .getLogger()
                                            .warning("Found unknown enchant: " + newId);
                                    }
                                    continue;
                                }
            ...
            if (!storedEnch && !hasValidEnchants) {
                ...
                if (hideFlags == null) {
                    ...
                    tag.putInt(this.extraNbtTag + "|OldHideFlags", hideFlags.asByte());
                }
        ...
        if (tag != null) {
            ...
            if (display != null) {
                ...
                if (name != null) {
                    ...
                    name.setValue(
                        via instanceof StringTag
                            ? ((StringTag)via).getValue()
                            : ComponentUtil.legacyToJsonString(name.getValue())
                    );
                }
            }
    ...
    private void rewriteEnchantmentsToServer(CompoundTag tag, boolean storedEnch) {
        ...
        if (enchantments != null) {
            ...
            if (!storedEnch) {
                Tag hideFlags = tag.remove(this.extraNbtTag + "|OldHideFlags");
            ...
            ListTag<CompoundTag> noMapped = tag.getListTag(this.extraNbtTag + "|Enchantments", CompoundTag.class);
            if (noMapped != null) {
            ...
            ListTag<StringTag> oldLore = tag.getListTag(this.extraNbtTag + "|OldLore", StringTag.class);
            if (oldLore != null) {
    ...
    private static void flowerPotSpecialTreatment(UserConnection user, int blockState, Position position) throws Exception {
        if (FlowerPotHandler.isFlowah(blockState)) {
            BackwardsBlockEntityProvider beProvider = Via.getManager()
                .getProviders()
                .get(BackwardsBlockEntityProvider.class);
