...
public class EntityPackets1_13 extends LegacyEntityRewriter<ClientboundPackets1_13, Protocol1_12_2To1_13> {
    ...
    protected void registerPackets() {
        this.protocol
            .registerClientbound(
                ClientboundPackets1_13.PLAYER_POSITION,
                new PacketHandlers() {
                    @Override
                        ...
                        this.handler(
                            wrapper -> {
                                if (ViaBackwards.getConfig().isFix1_13FacePlayer()) {
                                    PlayerPositionStorage1_13 playerStorage = wrapper.user()
                                        .get(PlayerPositionStorage1_13.class);
                                    byte bitField = wrapper.get(Type.BYTE, 0);
                                    playerStorage.setX(
                                        EntityPackets1_13.toSet(
                                            bitField, 0, playerStorage.getX(), wrapper.get(Type.DOUBLE, 0)
                                        )
                                    );
                                    playerStorage.setY(
                                        EntityPackets1_13.toSet(
                                            bitField, 1, playerStorage.getY(), wrapper.get(Type.DOUBLE, 1)
                                        )
                                    );
                                    playerStorage.setZ(
                                        EntityPackets1_13.toSet(
                                            bitField, 2, playerStorage.getZ(), wrapper.get(Type.DOUBLE, 2)
                                        )
                                    );
                                }
                            }
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_13.SPAWN_ENTITY,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        ...
                        this.handler(
                            wrapper -> {
                                Optional<EntityTypes1_13.ObjectType> optionalType = EntityTypes1_13.ObjectType.findById(
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_13.SPAWN_MOB,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.VAR_INT);
                        ...
                        this.handler(
                            wrapper -> {
                                ...
                                EntityPackets1_13.this.<EntityTracker>tracker(wrapper.user())
                                    .addEntity(wrapper.get(Type.VAR_INT, 0), entityType);
                                ...
                                if (oldId == -1) {
                                    if (!EntityPackets1_13.this.hasData(entityType)) {
                                        ViaBackwards.getPlatform()
                                            .getLogger()
                                            .warning(
                                                "Could not find 1.12 entity type for 1.13 entity type "
                                                    + type
                                                    + "/"
                                                    + entityType
                                            );
                        ...
                        this.handler(EntityPackets1_13.this.getMobSpawnRewriter1_11(Types1_12.METADATA_LIST));
                    }
                }
            );
        this.protocol
            .registerClientbound(
                ClientboundPackets1_13.SPAWN_PLAYER,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.VAR_INT);
                        ...
                        this.map(Types1_13.METADATA_LIST, Types1_12.METADATA_LIST);
                        this.handler(
                            EntityPackets1_13.this.getTrackerAndMetaHandler(
                                Types1_12.METADATA_LIST, EntityTypes1_13.EntityType.PLAYER
                            )
                        );
                    }
                }
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_13.SPAWN_PAINTING,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.VAR_INT);
                        this.map(Type.UUID);
                        this.handler(
                            EntityPackets1_13.this.getTrackerHandler(EntityTypes1_13.EntityType.PAINTING, Type.VAR_INT)
                        );
        ...
        this.registerMetadataRewriter(
            ClientboundPackets1_13.ENTITY_METADATA, Types1_13.METADATA_LIST, Types1_12.METADATA_LIST
        );
        this.protocol
            .registerClientbound(
        ...
        if (ViaBackwards.getConfig().isFix1_13FacePlayer()) {
            PacketHandlers movementRemapper = new PacketHandlers() {
                ...
                public void register() {
                    ...
                    this.map(Type.DOUBLE);
                    this.handler(
                        wrapper -> wrapper.user().get(PlayerPositionStorage1_13.class).setCoordinates(wrapper, false)
                    );
                }
            };
    ...
    protected void registerRewrites() {
        this.mapEntityTypeWithData(EntityTypes1_13.EntityType.DROWNED, EntityTypes1_13.EntityType.ZOMBIE_VILLAGER)
            .plainName();
        ...
        this.mapEntityTypeWithData(EntityTypes1_13.EntityType.TROPICAL_FISH, EntityTypes1_13.EntityType.SQUID)
            .plainName();
            ...
            .handler(
                (event, meta) -> {
                    ...
                    if (typeId == 4) {
                        ...
                        this.protocol.translatableRewriter().processText(event.user(), element);
                        meta.setMetaType(MetaType1_12.Chat);
                    } else if (typeId == 5) {
                        JsonElement element = meta.value();
                        meta.setTypeAndValue(MetaType1_12.String, this.protocol.jsonToLegacy(event.user(), element));
                    } else if (typeId == 6) {
                        Item item = (Item)meta.getValue();
                        meta.setTypeAndValue(
                            MetaType1_12.Slot, this.protocol.getItemRewriter().handleItemToClient(event.user(), item)
                        );
                    } else if (typeId == 15) {
                        event.cancel();
                    } else {
