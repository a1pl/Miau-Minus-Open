...
public class PlayerPacket1_13 extends RewriterBase<Protocol1_12_2To1_13> {
    ...
    protected void registerPackets() {
        this.protocol.registerClientbound(State.LOGIN, 4, -1, new PacketHandlers() {
            ...
            public void register() {
                this.handler(packetWrapper -> {
                    ...
                    packetWrapper.create(2, wrapper -> {
                        wrapper.write(Type.VAR_INT, packetWrapper.read(Type.VAR_INT));
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_13.PLUGIN_MESSAGE,
                wrapper -> {
                    ...
                    if (channel.equals("minecraft:trader_list")) {
                        ...
                        for (int i = 0; i < size; i++) {
                            ...
                            wrapper.write(
                                Type.ITEM1_8, this.protocol.getItemRewriter().handleItemToClient(wrapper.user(), input)
                            );
                            Item output = wrapper.read(Type.ITEM1_13);
                            wrapper.write(
                                Type.ITEM1_8,
                                this.protocol.getItemRewriter().handleItemToClient(wrapper.user(), output)
                            );
                            ...
                            if (secondItem) {
                                ...
                                wrapper.write(
                                    Type.ITEM1_8,
                                    this.protocol.getItemRewriter().handleItemToClient(wrapper.user(), second)
                                );
                            }
                        ...
                        if (oldChannel == null) {
                            if (!Via.getConfig().isSuppressConversionWarnings() || Via.getManager().isDebug()) {
                                ViaBackwards.getPlatform()
                                    .getLogger()
                                    .warning("Ignoring clientbound plugin message with channel: " + channel);
                            }
                        ...
                        if (oldChannel.equals("REGISTER") || oldChannel.equals("UNREGISTER")) {
                            String[] channels = new String(wrapper.read(Type.REMAINING_BYTES), StandardCharsets.UTF_8)
                                .split("\u0000");
                            ...
                            for (String s : channels) {
                                ...
                                if (rewritten != null) {
                                    ...
                                    ViaBackwards.getPlatform()
                                        .getLogger()
                                        .warning("Ignoring plugin channel in clientbound " + oldChannel + ": " + s);
                                }
                            }

                            wrapper.write(
                                Type.REMAINING_BYTES,
                                Joiner.on('\u0000').join(rewrittenChannels).getBytes(StandardCharsets.UTF_8)
                            );
                        }
                    }
                }
            ...
            public void register() {
                ...
                this.handler(wrapper -> {
                    ...
                    if (mode == 0 || mode == 2) {
                        ...
                        String legacyValue = PlayerPacket1_13.this.protocol.jsonToLegacy(wrapper.user(), value);
                        ...
                        this.handler(
                            wrapper -> {
                                ...
                                if (action == 0 || action == 2) {
                                    ...
                                    String legacyTextDisplayName = PlayerPacket1_13.this.protocol
                                        .jsonToLegacy(wrapper.user(), displayName);
                                    ...
                                    String prefix = PlayerPacket1_13.this.protocol
                                        .jsonToLegacy(wrapper.user(), prefixComponent);
                                    if (ViaBackwards.getConfig().addTeamColorTo1_13Prefix()) {
                                        prefix = prefix
                                            + "§"
                                            + (colour > -1 && colour <= 15 ? Integer.toHexString(colour) : "r");
                                    }

                                    String suffix = PlayerPacket1_13.this.protocol
                                        .jsonToLegacy(wrapper.user(), suffixComponent);
                ...
                wrapper -> {
                    ...
                    switch (channel) {
                        ...
                        case "MC|BEdit":
                            ...
                            wrapper.write(
                                Type.ITEM1_13, this.protocol.getItemRewriter().handleItemToServer(wrapper.user(), book)
                            );
                        ...
                        case "MC|AdvCmd":
                            ...
                            if (type == 0) {
                                ...
                                ViaBackwards.getPlatform()
                                    .getLogger()
                                    .warning("Client send MC|AdvCmd custom payload to update command block, weird!");
                            } else if (type == 1) {
                        ...
                        case "MC|Struct":
                            ...
                            String mode = wrapper.read(Type.STRING);
                            int modeId = mode.equals("SAVE")
                                ? 0
                                : (mode.equals("LOAD") ? 1 : (mode.equals("CORNER") ? 2 : 3));
                            wrapper.write(Type.VAR_INT, modeId);
                            wrapper.passthrough(Type.STRING);
                            ...
                            if (mode.equals("NONE")) {
                                ...
                                int mirrorId = mode.equals("LEFT_RIGHT") ? 1 : 2;
                            }
                            ...
                            if (mode.equals("NONE")) {
                                ...
                                int rotationId = mode.equals("CLOCKWISE_180") ? 2 : 3;
                            }
                        ...
                        default:
                            ...
                            if (newChannel == null) {
                                if (!Via.getConfig().isSuppressConversionWarnings() || Via.getManager().isDebug()) {
                                    ViaBackwards.getPlatform()
                                        .getLogger()
                                        .warning("Ignoring serverbound plugin message with channel: " + channel);
                                }
                                ...
                                for (String s : channels) {
                                    ...
                                    if (rewritten != null) {
                                        ...
                                        ViaBackwards.getPlatform()
                                            .getLogger()
                                            .warning(
                                                "Ignoring plugin channel in serverbound "
                                                    + Key.stripMinecraftNamespace(newChannel).toUpperCase(Locale.ROOT)
                                                    + ": "
                                                    + s
                                            );
                                    }
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_13.STATISTICS,
                new PacketHandlers() {
                    @Override
                    public void register() {
                                ...
                                for (int i = 0; i < size; i++) {
                                    ...
                                    switch (categoryId) {
                                        ...
                                        case 7:
                                            ...
                                            break;
                                        case 8:
                                            name = PlayerPacket1_13.this.protocol
                                                .getMappingData()
