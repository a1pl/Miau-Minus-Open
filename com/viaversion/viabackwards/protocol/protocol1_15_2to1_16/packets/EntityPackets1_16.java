...
public class EntityPackets1_16 extends EntityRewriter<ClientboundPackets1_16, Protocol1_15_2To1_16> {
    private final ValueTransformer<String, Integer> dimensionTransformer = new ValueTransformer<String, Integer>(
    ...
    protected void registerPackets() {
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_16.JOIN_GAME,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.INT);
                        ...
                        this.handler(
                            wrapper -> {
                                ...
                                EntityPackets1_16.this.<EntityTracker>tracker(wrapper.user())
                                    .addEntity(wrapper.get(Type.INT, 0), EntityTypes1_16.PLAYER);
                                wrapper.write(Type.STRING, "default");
        ...
        this.registerMetadataRewriter(
            ClientboundPackets1_16.ENTITY_METADATA, Types1_16.METADATA_LIST, Types1_14.METADATA_LIST
        );
        this.protocol.registerClientbound(ClientboundPackets1_16.ENTITY_PROPERTIES, wrapper -> {
            ...
            for (int i = 0; i < size; i++) {
                ...
                String oldKey = this.protocol.getMappingData().attributeIdentifierMappings().get(attributeIdentifier);
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_16.PLAYER_INFO,
                wrapper -> {
                        ...
                        if (action != 0) {
                            if (action == 1) {
                                ...
                                this.protocol
                                    ...
                                    .processText(wrapper.user(), wrapper.passthrough(Type.OPTIONAL_COMPONENT));
                            }
                        } else {
    ...
    protected void registerRewrites() {
        this.filter().handler((event, meta) -> {
            ...
            if (type == Types1_14.META_TYPES.itemType) {
                meta.setValue(this.protocol.getItemRewriter().handleItemToClient(event.user(), (Item)meta.getValue()));
                ...
                this.rewriteParticle(event.user(), (Particle)meta.getValue());
