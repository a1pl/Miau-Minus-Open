...
public class BlockItemPackets1_14
    extends BackwardsItemRewriter<ClientboundPackets1_14, ServerboundPackets1_13, Protocol1_13_2To1_14> {
    ...
    protected void registerPackets() {
        this.protocol
            .registerServerbound(
                ServerboundPackets1_13.EDIT_BOOK,
                ...
                wrapper -> {
                    ...
                    if (stringType == null) {
                        ViaBackwards.getPlatform()
                            .getLogger()
                            .warning("Can't open inventory for 1.13 player! Type: " + type);
                        wrapper.cancel();
                    } else {
                        ...
                        if (containerTitle != null
                            ...
                            && (
                                type != 2
                                    || object.getAsJsonPrimitive("translate").getAsString().equals("container.barrel")
                            )) {
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_14.OPEN_HORSE_WINDOW, ClientboundPackets1_13.OPEN_WINDOW, wrapper -> {
        ...
        this.protocol
            .registerClientbound(ClientboundPackets1_14.TRADE_LIST, ClientboundPackets1_13.PLUGIN_MESSAGE, wrapper -> {
        ...
        this.protocol
            .registerClientbound(ClientboundPackets1_14.OPEN_BOOK, ClientboundPackets1_13.PLUGIN_MESSAGE, wrapper -> {
                wrapper.write(Type.STRING, "minecraft:book_open");
                wrapper.passthrough(Type.VAR_INT);
            });
        this.protocol
            .registerClientbound(
                ClientboundPackets1_14.ENTITY_EQUIPMENT,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        ...
                        this.handler(
                            wrapper -> BlockItemPackets1_14.this.handleItemToClient(
                                wrapper.user(), wrapper.get(Type.ITEM1_13_2, 0)
                            )
                        );
                        this.handler(
                            wrapper -> {
                                int entityId = wrapper.get(Type.VAR_INT, 0);
                                EntityType entityType = wrapper.user()
                                    .<EntityTracker>getEntityTracker(Protocol1_13_2To1_14.class)
                                    .entityType(entityId);
                                if (entityType != null) {
                                    if (entityType.isOrHasParent(EntityTypes1_14.ABSTRACT_HORSE)) {
                                        ...
                                        int armorType = item != null && item.identifier() != 0
                                            ? item.identifier() - 726
                                            : 0;
        ...
        Set<String> removedTypes = ImmutableSet.of(
            "crafting_special_suspiciousstew", "blasting", "smoking", "campfire_cooking", "stonecutting"
        );
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_14.BLOCK_ACTION,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.POSITION1_14, Type.POSITION1_8);
                        ...
                        this.handler(
                            wrapper -> {
                                int mappedId = BlockItemPackets1_14.this.protocol
                                    .getMappingData()
                                    .getNewBlockId(wrapper.get(Type.VAR_INT, 0));
                                if (mappedId == -1) {
                                    wrapper.cancel();
                                } else {
                        ...
                        this.handler(
                            wrapper -> {
                                ...
                                wrapper.set(
                                    Type.VAR_INT,
                                    0,
                                    BlockItemPackets1_14.this.protocol.getMappingData().getNewBlockStateId(id)
                                );
                            }
                        );
                    }
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_14.CHUNK_DATA,
                wrapper -> {
                    ...
                    ChunkLightStorage.ChunkLight chunkLight = wrapper.user()
                        .get(ChunkLightStorage.class)
                        .getStoredLight(chunk.getX(), chunk.getZ());
                    ...
                    for (int i = 0; i < chunk.getSections().length; i++) {
                        ...
                        if (section != null) {
                            ...
                            if (chunkLight == null) {
                                ...
                                sectionLight.setBlockLight(
                                    blockLight != null ? blockLight : ChunkLightStorage.FULL_LIGHT
                                );
                                if (clientWorld.getEnvironment() == Environment.NORMAL) {
                            ...
                            if (Via.getConfig().isNonFullBlockLightFix()
                                && section.getNonAirBlocksCount() != 0
                                && sectionLight.hasBlockLight()) {
                                for (int x = 0; x < 16; x++) {
                                    for (int y = 0; y < 16; y++) {
                                        for (int z = 0; z < 16; z++) {
                            ...
                            wrapper -> {
                                ...
                                if (id == 1010) {
                                    wrapper.set(
                                        Type.INT,
                                        1,
                                        BlockItemPackets1_14.this.protocol.getMappingData().getNewItemId(data)
                                    );
                                } else if (id == 2001) {
                                    wrapper.set(
                                        Type.INT,
                                        1,
                                        BlockItemPackets1_14.this.protocol.getMappingData().getNewBlockStateId(data)
                                    );
                                }
                            }
                        );
