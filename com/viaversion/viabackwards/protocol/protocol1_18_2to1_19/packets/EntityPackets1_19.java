...
public final class EntityPackets1_19 extends EntityRewriter<ClientboundPackets1_19, Protocol1_18_2To1_19> {
    ...
    protected void registerPackets() {
        ...
        this.registerMetadataRewriter(
            ClientboundPackets1_19.ENTITY_METADATA, Types1_19.METADATA_LIST, Types1_18.METADATA_LIST
        );
                        ...
                        this.handler(
                            wrapper -> {
                                ...
                                if (entityType.isOrHasParent(EntityTypes1_19.LIVINGENTITY)) {
                                    ...
                                    StoredEntityData entityData = EntityPackets1_19.this.<EntityTracker>tracker(
                                            wrapper.user()
                                        )
                                        .entityData(entityId);
                                    Position position = new Position(
                                        wrapper.get(Type.DOUBLE, 0).intValue(),
                                        wrapper.get(Type.DOUBLE, 1).intValue(),
                                        wrapper.get(Type.DOUBLE, 2).intValue()
                                    );
                                    entityData.put(
                                        new StoredPainting(entityId, wrapper.get(Type.UUID, 0), position, data)
                                    );
                                } else {
                                    if (entityType == EntityTypes1_19.FALLING_BLOCK) {
                                        data = EntityPackets1_19.this.protocol
                                            .getMappingData()
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_19.JOIN_GAME,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.map(Type.INT);
                        ...
                        this.handler(
                            wrapper -> {
                                DimensionRegistryStorage dimensionRegistryStorage = wrapper.user()
                                    .get(DimensionRegistryStorage.class);
                                dimensionRegistryStorage.clear();
                                String dimensionKey = Key.stripMinecraftNamespace(wrapper.read(Type.STRING));
                                ...
                                for (CompoundTag dimension : dimensions) {
                                    StringTag nameTag = dimension.getStringTag("name");
                                ...
                                if (!found) {
                                    throw new IllegalStateException(
                                        "Could not find dimension " + dimensionKey + " in dimension registry"
                                    );
                                }
                                ...
                                EntityPackets1_19.this.<EntityTracker>tracker(wrapper.user())
                                    .setBiomesSent(biomes.size());
                                CompoundTag chatTypeRegistry = registry.remove("minecraft:chat_type");
                                if (chatTypeRegistry == null) {
                                    chatTypeRegistry = registry.remove("chat_type");
                                }

                                for (CompoundTag chatType : chatTypeRegistry.getListTag("value", CompoundTag.class)) {
        ...
        this.protocol
            .registerClientbound(
                ClientboundPackets1_19.RESPAWN,
                new PacketHandlers() {
                    @Override
                    public void register() {
                        this.handler(
                            wrapper -> {
                                ...
                                CompoundTag dimension = wrapper.user()
                                    .get(DimensionRegistryStorage.class)
    ...
    protected void registerRewrites() {
        ...
        this.filter()
            .type(EntityTypes1_19.PAINTING)
            .index(8)
            .handler(
                (event, meta) -> {
                    event.cancel();
                    StoredEntityData entityData = this.<EntityTracker>tracker(event.user())
                        .entityDataIfPresent(event.entityId());
        ...
        this.filter()
            .type(EntityTypes1_19.CAT)
            .index(19)
            .handler((event, meta) -> meta.setMetaType(Types1_18.META_TYPES.varIntType));
