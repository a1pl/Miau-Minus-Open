...
public final class Protocol1_18_2To1_19
    ...
    private final TranslatableRewriter<ClientboundPackets1_19> translatableRewriter = new TranslatableRewriter<>(
    ...
    protected void registerPackets() {
        ...
        this.tagRewriter.registerGeneric(ClientboundPackets1_19.TAGS);
        ...
        this.registerClientbound(
            ClientboundPackets1_19.DECLARE_COMMANDS,
            wrapper -> {
                    ...
                    if (nodeType == 2) {
                        ...
                        if (argumentType == null) {
                            ViaBackwards.getPlatform()
                                .getLogger()
                                .warning("Unknown command argument type id: " + argumentTypeId);
                ...
                public void register() {
                    this.handler(
                        wrapper -> {
                            ...
                            CompoundTag chatType = wrapper.user()
                            ...
                            ChatDecorationResult decorationResult = Protocol1_19_1To1_19.decorateChatMessage(
                                ...
                                senderName,
                                teamName,
                                unsignedContent != null ? unsignedContent : signedContent
                            );
                            if (decorationResult == null) {
                                wrapper.cancel();
                            } else {
                                Protocol1_18_2To1_19.this.translatableRewriter
                                    .processText(wrapper.user(), decorationResult.content());
        ...
        this.registerClientbound(
            ClientboundPackets1_19.SYSTEM_CHAT, ClientboundPackets1_18.CHAT_MESSAGE, new PacketHandlers() {
                @Override
                public void register() {
                    this.handler(wrapper -> {
                        JsonElement content = wrapper.passthrough(Type.COMPONENT);
                        Protocol1_18_2To1_19.this.translatableRewriter.processText(wrapper.user(), content);
        ...
        this.registerServerbound(
            ServerboundPackets1_17.CHAT_MESSAGE,
            new PacketHandlers() {
                @Override
                public void register() {
                    this.map(Type.STRING);
                        ...
                        wrapper -> {
                            ...
                            if (!message.isEmpty() && message.charAt(0) == '/') {
                                ...
                                if (chatSession != null && argumentsProvider != null) {
                                    ...
                                    List<Pair<String, String>> arguments = argumentsProvider.getSignableArguments(
                                        command
                                    );
                                    ...
                                    for (Pair<String, String> argument : arguments) {
                                        byte[] signature = chatSession.signChatMessage(
                                            metadata, new DecoratableMessage(argument.value())
                                        );
                ...
                public void register() {
                    ...
                    this.handler(wrapper -> {
                        ...
                        for (int i = 0; i < properties; i++) {
                            ...
                            wrapper.read(Type.STRING);
                            wrapper.read(Type.OPTIONAL_STRING);
                        }
                    });
                }
            }
