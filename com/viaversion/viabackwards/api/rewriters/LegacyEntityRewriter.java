...
public abstract class LegacyEntityRewriter<C extends ClientboundPacketType, T extends BackwardsProtocol<C, ?, ?, ?>>
    ...
    protected EntityObjectData mapObjectType(ObjectType oldObjectType, ObjectType replacement, int data) {
        EntityObjectData entData = new EntityObjectData(
            this.protocol, oldObjectType.getType().name(), oldObjectType.getId(), replacement.getId(), data
        );
    ...
    public void registerMetadataRewriter(
        C packetType, Type<List<Metadata>> oldMetaType, Type<List<Metadata>> newMetaType
    ) {
        this.protocol.registerClientbound(packetType, new PacketHandlers() {
            @Override
            public void register() {
    ...
    protected PacketHandler getMobSpawnRewriter(Type<List<Metadata>> metaType, LegacyEntityRewriter.IdSetter idSetter) {
        return wrapper -> {
    ...
    public PacketHandler getMobSpawnRewriter(Type<List<Metadata>> metaType) {
        return this.getMobSpawnRewriter(metaType, (wrapper, id) -> wrapper.set(Type.UNSIGNED_BYTE, 0, (short)id));
    }

    public PacketHandler getMobSpawnRewriter1_11(Type<List<Metadata>> metaType) {
        return this.getMobSpawnRewriter(metaType, (wrapper, id) -> wrapper.set(Type.VAR_INT, 1, id));
    }

    protected PacketHandler getObjectTrackerHandler() {
        return wrapper -> this.addTrackedEntity(
            wrapper, wrapper.get(Type.VAR_INT, 0), this.objectTypeFromId(wrapper.get(Type.BYTE, 0))
        );
    }
        ...
        return wrapper -> {
            ...
            if (type == null) {
                ViaBackwards.getPlatform()
                    .getLogger()
                    .warning("Could not find Entity Type" + wrapper.get(Type.BYTE, 0));
            } else {
    ...
    protected interface IdSetter {
        void setId(PacketWrapper var1, int var2) throws Exception;
    }
}
