...
public abstract class EntityRewriterBase<C extends ClientboundPacketType, T extends BackwardsProtocol<C, ?, ?, ?>>
    ...
    EntityRewriterBase(
        T protocol,
        MetaType displayNameMetaType,
        int displayNameIndex,
        MetaType displayVisibilityMetaType,
        int displayVisibilityIndex
    ) {
        ...
        if (entity != null) {
            ...
            if (entityData != null && (displayNameObject = entityData.entityName()) != null) {
                ...
                if (initialMetadata) {
                    if (displayName == null) {
                        metadataList.add(
                            new Metadata(this.displayNameIndex, this.displayNameMetaType, displayNameObject)
                        );
                    ...
                    && (displayName.getValue() == null || displayName.getValue().toString().isEmpty())) {
                    displayName.setValue(displayNameObject);
                    this.addDisplayVisibilityMeta(metadataList);
                }
            }
    ...
    private void addDisplayVisibilityMeta(List<Metadata> metadataList) {
        if (this.alwaysShowOriginalMobName()) {
            ...
            metadataList.add(
                new Metadata(
                    this.displayVisibilityIndex, this.displayVisibilityMetaType, this.getDisplayVisibilityMetaValue()
                )
            );
        }
    }
    ...
    protected EntityData mapEntityTypeWithData(EntityType type, EntityType mappedType) {
        Preconditions.checkArgument(
            type.getClass() == mappedType.getClass(), "Both entity types need to be of the same class"
        );
    ...
    public void registerMetaTypeHandler(
        ...
        this.filter().handler((event, meta) -> {
            ...
            if (type == itemType) {
                this.protocol.getItemRewriter().handleItemToClient(event.user(), meta.value());
                ...
                this.protocol.getTranslatableRewriter().processText(event.user(), text);
            }
        });
    }
    ...
    public void registerMetaTypeHandler1_20_3(
        ...
        this.filter().handler((event, meta) -> {
            ...
            if (type == itemType) {
                ...
                this.protocol.getTranslatableRewriter().processTag(event.user(), meta.value());
            }
        });
    }
    ...
    protected PacketHandler getTrackerHandler(Type<? extends Number> intType, int typeIndex) {
        return wrapper -> {
            ...
            this.<EntityTracker>tracker(wrapper.user())
                .addEntity(wrapper.get(Type.VAR_INT, 0), this.typeFromId(id.intValue()));
        };
    }
    ...
    protected PacketHandler getTrackerHandler(EntityType entityType, Type<? extends Number> intType) {
        return wrapper -> this.<EntityTracker>tracker(wrapper.user())
            .addEntity((Integer)((Number)wrapper.get(intType, 0)), entityType);
    }
