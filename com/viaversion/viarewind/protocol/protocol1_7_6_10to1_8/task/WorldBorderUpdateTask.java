            ...
            if (worldBorderEmulatorTracker.isInit()) {
                ...
                for (WorldBorderEmulator.Side side : WorldBorderEmulator.Side.values()) {
                    ...
                    if (!(d >= 16.0)) {
                        ...
                        PacketWrapper spawnParticle = PacketWrapper.create(
                            ClientboundPackets1_8.SPAWN_PARTICLE, connection
                        );
                        spawnParticle.write(Type.STRING, ViaRewind.getConfig().getWorldBorderParticle());
                        spawnParticle.write(
                            Type.FLOAT,
                            (float)(side.modX != 0 ? worldBorderEmulatorTracker.getX() + radius * side.modX : centerH)
                        );
                        spawnParticle.write(Type.FLOAT, (float)centerV);
                        spawnParticle.write(
                            Type.FLOAT,
                            (float)(side.modX == 0 ? worldBorderEmulatorTracker.getZ() + radius * side.modZ : centerH)
                        );
                        ...
                        try {
                            ...
                            ViaRewind.getPlatform()
                                .getLogger()
                                .log(Level.SEVERE, "Failed to send world border particle", e);
                        }
                    }
                }
            }
