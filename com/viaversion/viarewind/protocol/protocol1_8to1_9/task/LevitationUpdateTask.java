    ...
    public void run() {
        for (UserConnection connection : Via.getManager().getConnectionManager().getConnections()) {
            ...
            if (levitation.isActive()) {
                PacketWrapper velocityPacket = PacketWrapper.create(ClientboundPackets1_8.ENTITY_VELOCITY, connection);
                velocityPacket.write(
                    Type.VAR_INT, connection.<EntityTracker>getEntityTracker(Protocol1_8To1_9.class).clientEntityId()
                );
