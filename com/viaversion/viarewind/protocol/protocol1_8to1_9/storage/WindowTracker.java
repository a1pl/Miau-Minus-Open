...
public class WindowTracker extends StoredObject {
    ...
    public Item[] getBrewingItems(short windowId) {
        return this.brewingItems
            .computeIfAbsent(
                windowId, key -> new Item[]{new DataItem(), new DataItem(), new DataItem(), new DataItem()}
            );
    }

    public short getEnchantmentValue(short key) {
        return !this.enchantmentProperties.containsKey(key) ? 0 : this.enchantmentProperties.remove(key);
    }
